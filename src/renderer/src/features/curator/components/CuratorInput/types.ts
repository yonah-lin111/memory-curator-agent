...
import type {
  ...
  CuratorInputCommand,
  CuratorInputCommandId,
  SelectedTextFile,
} from "@/lib/ai-shared/types"

export type { AgentMentionPanelState, CuratorInputCommand, CuratorInputCommandId, SelectedTextFile }
...
export interface CuratorInputProps {
  ...
  // 执行输入框斜杠命令回调。
  onCommandExecute: (command: CuratorInputCommandId) => string | void | Promise<string | void>
  // AI 模型切换回调。
