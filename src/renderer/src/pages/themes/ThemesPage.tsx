...
import { Activity, Archive, Download, FileText, Layers, RotateCcw, Sparkles } from "lucide-react"
...
export const ThemesPage = (): React.JSX.Element => {
  ...
  // Tooltip 内联编辑状态
  const [editTooltipTheme, setEditTooltipTheme] = useState<ThemeItem | null>(null)
    ...
    window.api.themes
      ...
      .then((itemsResult) => {
        ...
        itemsResult.forEach((item) => {
          const dateStr = (item.sourceEntryDate || item.createdAt)
            .substring(0, 10)
            .replace(/\//g, "-")
  ...
  const archivedThemes = useMemo(() => themes.filter((t) => t.status === "archived"), [themes])
  ...
  const handleTooltipSave = async (): Promise<void> => {
    if (!editTooltipTheme || !editTooltipName.trim() || !window.api?.themes) return
    try {
  ...
  useEffect(() => {
    ...
    const seriesData = timeline.map((t) => ({
      value: t.itemCount,
      itemStyle: {
        color: t.mentionedInSummary ? "rgba(74, 222, 128, 1)" : "rgba(255, 255, 255, 0.8)",
      },
    }))
          ...
          <div className="flex items-center justify-between border-b border-white/5 pb-2 flex-shrink-0">
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-2 mr-1">
                <span className="text-sm font-bold text-white/80">主题列表</span>
              ...
              <Tooltip
                ...
                form={
                  <div className="flex flex-col gap-2">
                    <span className="text-xs font-semibold text-white/40">标签 (逗号分隔)</span>
                    <input
                ...
                form={
                  <div className="flex flex-col gap-3">
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-semibold text-white/40">主题名称</span>
                      <input
                        className="w-full bg-[#212121] border border-white/10 rounded-[6px] px-3 py-2 text-xs text-white/90 placeholder:text-white/20 outline-none focus:border-white/30 transition-colors"
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-semibold text-white/40">描述</span>
                      <textarea
                            ...
                            <div className="flex flex-col gap-3">
                              <div className="flex flex-col gap-1">
                                ...
                                <input
                                  ...
                                  onChange={(e) => setEditTooltipName(e.target.value)}
                                  placeholder="主题名称"
                                  autoFocus
                                />
                              </div>
                              <div className="flex flex-col gap-1">
                                ...
                                <textarea
                                  ...
                                  onChange={(e) => setEditTooltipDesc(e.target.value)}
                                  placeholder="意图描述"
                                />
                              </div>
                            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 min-h-[220px] flex-shrink-0">
                ...
                <div className="xl:col-span-2 bg-[#1a1a1a] rounded-[6px] border border-white/5 p-6 flex flex-col justify-between relative group overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <Tag
                        size="small"
                        color={selectedTheme.status === "active" ? "emerald" : "default"}
                  ...
                  <div className="flex flex-wrap items-center gap-5 mt-6 border-t border-white/5 pt-4">
                    <div className="flex flex-col">
                      <span className="text-xs font-mono text-white/30 mb-1">RECORDED ITEMS</span>
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-mono text-white/30 mb-1">CREATED AT</span>
                    ...
                    <div className="w-px h-6 bg-white/5" />
                    <div className="flex flex-col">
                      <span className="text-xs font-mono text-white/30 mb-1">LAST UPDATE</span>
                      <span className="text-xs text-white/60 font-mono mt-0.5">
                ...
                <div className="xl:col-span-1 bg-[#1a1a1a] rounded-[6px] border border-white/5 p-5 flex flex-col relative overflow-hidden">
                  ...
                  <div className="flex-1 w-full relative min-h-[200px] z-10" ref={timelineChartRef}>
                    {timeline.length === 0 && (
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-xs text-white/20 font-mono">暂无脉搏数据</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-[#1a1a1a] rounded-[6px] border border-white/5 p-6 flex flex-col flex-1">
                ...
                {itemsLoading ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center border border-white/5 border-dashed rounded-[6px] bg-black/20 m-2 min-h-[160px]">
                    ...
                    <span className="text-xs text-white/30 font-mono">该主题下尚无碎片记录</span>
                  </div>
                ) : (
