...
const buildStructuredWhere = (parsed: NoteQueryToolInput): string => {
  const whereParts: (string | null)[] = [
    ...
    typeof parsed.categoryId === "number" ? `n.category_id = ${parsed.categoryId}` : null,
    parsed.query
...
const prepareNoteSql = (sql: string, limit: number): string => {
  ...
  if (normalizedSql.includes(";") || SQL_COMMENT_PATTERN.test(normalizedSql)) {
    throw new Error("Note SQL only allows a single SELECT statement without comments")
  }
  ...
  if (
    /\bfrom\s+(?!notes\b|notes\s+n(\s+LEFT\s+JOIN\s+note_categories\s+nc)?)[a-z_][\w]*/i.test(
      normalizedSql,
    )
  ) {
...
const queryBySql = (
  ...
  const rows = notesService.querySql(prepareNoteSql(sql, limit)).slice(0, limit)
...
export const createNoteQueryTool = (
  ...
  description: "Query notes in the local Notes table. Read-only; never modifies data.",
  prompt: {
    summary:
      "Query notes in the local Notes table. Read-only; supports structured filters and controlled SQL.",
    intentKeywords: ["笔记", "速记", "随手记", "记录", "素材", "note", "notes", "query notes"],
    whenToUse: [
    ...
    output: "Return the note facts needed to answer the user. Do not repeat irrelevant fields.",
    examples: [
      `{"limit":10}`,
      `{"query":"React"}`,
      `{"tag":"前端","categoryId":1}`,
  ...
  execute: async (input) => {
    ...
    const limit = Math.max(1, Math.min(parsed.limit ?? DEFAULT_NOTE_LIMIT, MAX_NOTE_LIMIT))
 ...
 * 生成 Note 写入前确认说明。
 */
const renderNoteMutationSummary = (action: NoteWriteAction, input: unknown): string | null => {
...
const NOTE_ADD_CONFIRMATION: ToolConfirmationConfig = {
  ...
  completion: {
    renderMessage: (input, result) => renderNoteMutationCompletion("add", input, result),
  },
}
...
const NOTE_UPDATE_CONFIRMATION: ToolConfirmationConfig = {
  ...
  completion: {
    renderMessage: (input, result) => renderNoteMutationCompletion("update", input, result),
  },
}
...
const NOTE_DELETE_CONFIRMATION: ToolConfirmationConfig = {
  ...
  completion: {
    renderMessage: (input, result) => renderNoteMutationCompletion("delete", input, result),
  },
}
 ...
 * 解析 Note 更新入参。
 */
const parseUpdateInput = (input: unknown): { id: number; profile: NoteCreateInput } => {
  if (!isRecord(input)) {
 ...
 * 创建 Note 新建工具。
 */
export const createNoteAddTool = (notesService: Pick<NotesService, "create">): NoteWriteTool => ({
  name: "notes_tool_add",
  ...
  prompt: {
    ...
    intentKeywords: ["添加笔记", "新增笔记", "创建笔记", "记录笔记", "add note", "create note"],
    whenToUse: [
...
export const createNoteUpdateTool = (
  ...
  prompt: {
    ...
    intentKeywords: ["修改笔记", "更新笔记", "纠正笔记", "update note", "edit note"],
    whenToUse: [
...
export const createNoteDeleteTool = (
  ...
  prompt: {
    ...
    intentKeywords: ["删除笔记", "移除笔记", "删掉笔记", "delete note", "remove note"],
    whenToUse: [
    ...
    examples: ['{"confirmationSummary":"将删除笔记：**React 学习笔记**（聊天粘贴）。","id":1}'],
  },
  parameters: {
    type: "object",
 ...
 * 创建 Note 批量新建工具。
 */
const createNoteBatchAddTool = (notesService: Pick<NotesService, "create">): NoteWriteTool => ({
 ...
 * 创建 Note 批量更新工具。
 */
const createNoteBatchUpdateTool = (notesService: Pick<NotesService, "update">): NoteWriteTool => ({
  ...
  prompt: {
    ...
    intentKeywords: ["批量修改笔记", "批量更新笔记", "batch update notes", "batch edit notes"],
    whenToUse: [
 ...
 * 创建 Note 批量删除工具。
 */
const createNoteBatchDeleteTool = (notesService: Pick<NotesService, "delete">): NoteWriteTool => ({
