...
export const createProfileTools = (): AgentTool[] => [
  {
    ...
    prompt: {
      ...
      whenToUse: ['Use when the user asks about their saved personal information or when facts are needed before a profile update.'],
  ...
  },
  {
    ...
    prompt: {
      ...
      whenToUse: ['Use only when the user explicitly asks to create or save their personal profile and no profile exists.'],
      whenNotToUse: ['Do not use when a personal profile already exists; use profile_tool_update for changes.', 'Do not use to delete personal information.'],
      safety: ['The system will request internal confirmation before writing.', 'Require name and never invent profile facts.', 'Include confirmationSummary with the name and key facts being created.'],
    },
    parameters: {
      type: 'object',
      required: ['name'],
      properties: {
        confirmationSummary: { type: 'string', description: 'Concise Markdown Chinese summary shown in the confirmation.' },
    ...
    execute: async (input) => {
      ...
      return { observation: `Created personal profile: ${created.name}.`, data: { item: toToolItem(created) } }
    },
  },
  {
    ...
    prompt: {
      ...
      whenToUse: ['Use when the user explicitly asks to change, correct, or add saved personal information.'],
      whenNotToUse: ['Do not use to delete or clear the profile.', 'Do not use when the user did not request saving changes.'],
      safety: ['The system will request internal confirmation before writing.', 'Only provide fields explicitly requested by the user; omitted fields remain unchanged.', 'When no profile exists, name is required to create one.', 'Include confirmationSummary with the fields and facts being changed.'],
    },
    parameters: {
      type: 'object',
      properties: {
        ...
        birthday: { type: 'string', description: 'Birthday text.' },
        contact: { type: 'string', description: 'Contact details.' },
        tags: { type: 'array', items: { type: 'string' }, description: 'Complete personal tags to retain.' },
        details: { type: 'string', description: 'Personal details in Markdown.' },
      },
    },
    execute: async (input) => {
      ...
      return { observation: `Updated personal profile: ${updated.name}.`, data: { item: toToolItem(updated) } }
    },
  },
]
