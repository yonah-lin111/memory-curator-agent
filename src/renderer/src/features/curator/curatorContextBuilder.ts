import { resolveDedupedTextContents } from "@/features/curator/core/curatorReasoningDedupe"
import type {
  CuratorMessage,
...
// 共享工具函数，从 lib/ai-shared 引入本文件作用域并重导出。
import {
  estimateCuratorContextTokens,
  resolveCuratorSelectedModelOption,
} from "@/lib/ai-shared/contextBuilder"

export { estimateCuratorContextTokens, resolveCuratorSelectedModelOption }
...
// AI 对话上下文来源类型。
export type CuratorContextKind = "message" | "memory" | "page" | "file" | "tool" | "agent" | "skill"

// AI 对话上下文元信息类型。
export type CuratorContextMeta = Record<string, string | number | boolean | undefined>

// AI 对话上下文条目类型。
export type CuratorContextItem = {
  // 上下文稳定去重键。
  key: string
  // 所属 AI 会话标识。
  sessionId: string
  // 上下文来源类型。
  kind: CuratorContextKind
  // 来源对象标识。
  sourceId: string
  // 展示标题。
  title: string
  ...
  createdAt: number
  // 来源相关补充信息。
  meta: CuratorContextMeta
}

// AI 对话上下文预算输入类型。
type CuratorContextBudgetInput = {
  // 当前上下文条目列表。
...
// AI 对话上下文预算类型。
export type CuratorContextBudget = {
  // 上下文总 token 估算。
  totalTokens: number
  // 当前模型上下文窗口上限。
  contextLimit?: number
 ...
 * 从单条消息中提取可进入上下文的正文。
 */
const getMessageContextContent = (message: CuratorMessage): string => {
  if (message.role === "assistant" && message.parts?.length) {
    const textContent = resolveDedupedTextContents(message.parts).join("\n\n").trim()
 ...
 */
const isRunErrorAssistantMessage = (message: CuratorMessage): boolean => {
 ...
 */
const collectErroredQaMessageIds = (messages: CuratorMessage[]): Set<string> => {
...
export const buildMessageContextItems = (
  ...
  return messages.flatMap((message, index) => {
    ...
    const shouldSkipMessageContext =
      erroredQaMessageIds.has(message.id) || Boolean(message.cancelled)
    const items: CuratorContextItem[] = []

    if (message.role === "user" && content && !shouldSkipMessageContext) {
      let partsTokens = 0
      const meta: CuratorContextMeta = {
        role: message.role,
        time: message.time,
      }
      ...
      items.push({
        ...
        content,
        tokens: estimateCuratorContextTokens(content) + partsTokens,
        createdAt,
        meta,
      })
    }
    ...
    for (const [toolIndex, step] of (message.toolSteps ?? []).entries()) {
      ...
      items.push({
        ...
        content: toolContent,
        tokens: estimateCuratorContextTokens(toolContent),
    ...
    if (content && !shouldSkipMessageContext) {
      items.push({
        ...
        summary: summarizeContextContent(content),
        content,
        tokens: estimateCuratorContextTokens(content),
 ...
 * 计算当前上下文预算。
 */
export const getCuratorContextBudget = ({
  items,
  modelOptions,
  selectedModel,
}: CuratorContextBudgetInput): CuratorContextBudget => {
  const totalTokens = items.reduce((sum, item) => sum + item.tokens, 0)
  const model = resolveCuratorSelectedModelOption(modelOptions, selectedModel)
