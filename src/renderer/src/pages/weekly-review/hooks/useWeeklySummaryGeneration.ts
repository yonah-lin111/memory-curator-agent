...
// 单周状态映射。
type WeeklySummaryGenerationSnapshot = Record<
  string,
  WeeklySummaryGenerationState
>;
...
const updateWeekState = (
  weekStartDate: string,
  updater: (
    current: WeeklySummaryGenerationState,
  ) => WeeklySummaryGenerationState,
...
const ensureIpcListeners = (): void => {
  ...
  removeDeltaListener = window.api.weekly.summary.onDelta(
    ({ weekStartDate, text }) => {
...
const hydrateWeeklySummaryState = (
  ...
  if (
    existing?.panelState === "streaming" ||
    existing?.panelState === "loading"
  ) {
    return;
  }
  updateWeekState(
...
export const useWeeklySummaryGeneration = (weekStartDate: string) => {
  const generationSnapshot = useSyncExternalStore(
    subscribe,
    getSnapshot,
    getServerSnapshot,
  );
