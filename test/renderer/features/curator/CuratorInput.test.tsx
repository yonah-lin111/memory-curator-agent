...
const renderCuratorInput = (
  onCommandExecute: (
    command: CuratorInputCommandId,
  ) => string | void | Promise<string | void> = () => undefined,
  isGenerating = false,
  onSendMessage: (payload: CuratorSendPayload) => void = () => undefined,
  chatSessions: any[] = [],
...
describe("CuratorInput", () => {
  ...
  it("点击输入容器空白区域时聚焦文本框", () => {
    ...
    fireEvent.click(screen.getByTestId("curator-input-container"))
  ...
  it("输入 / 后保持输入框聚焦并支持键盘选择执行命令", async () => {
    ...
    renderCuratorInput(onCommandExecute)
    const textarea = screen.getByLabelText("AI Chat Input Area")
    textarea.focus()
  ...
  it("支持 /showContextTimeline 命令匹配与触发", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("option", { name: /\/showContextTimeline/ })).toBeInTheDocument(),
    )
    fireEvent.keyDown(textarea, {
      key: "Enter",
    })
  ...
  it("支持 /showFullScreen 命令匹配与触发", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("option", { name: /\/showFullScreen/ })).toBeInTheDocument(),
    )
    fireEvent.keyDown(textarea, {
      key: "Enter",
    })
  ...
  it("命令面板上下键循环选择", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("option", { name: /\/showFullScreen/ })).toHaveAttribute(
        "aria-selected",
        "true",
      )
    })
    ...
    await waitFor(() => {
      expect(screen.getByRole("option", { name: /\/clear/ })).toHaveAttribute(
        "aria-selected",
        "true",
      )
    })
  ...
  it("undo 命令返回文本时回填到输入框", async () => {
    renderCuratorInput((command) => (command === "undo" ? "需要重新编辑的问题" : undefined))
    const textarea = screen.getByLabelText("AI Chat Input Area")
    textarea.focus()
  ...
  it("发送时剥离 agent token 并保存包含 @ 命令的原始 prompt history", async () => {
    ...
    renderCuratorInput(() => undefined, false, onSendMessage)
    const textarea = screen.getByLabelText("AI Chat Input Area")
  ...
  it("斜杠命令面板打开时不打开 agent 面板", () => {
    ...
    expect(
      screen.queryByRole("listbox", { name: "AI Agent Mention Panel" }),
    ).not.toBeInTheDocument()
  })

  it("在 AI 正在输出时（isGenerating = true）尝试发送，会通过 Toast 提示并阻止发送", async () => {
    const onSendMessage = vi.fn()
    renderCuratorInput(() => undefined, true, onSendMessage)
    const textarea = screen.getByLabelText("AI Chat Input Area")
    textarea.focus()
  ...
  it("当选择的模型支持图片输入时，最多一次性上传 6 张图片，超出则警告并截断", async () => {
    const mockSaveCuratorImage = vi.fn().mockImplementation(({ name }) => {
    ...
    Object.defineProperty(window, "api", {
      ...
      value: {
        files: {
          saveCuratorImage: mockSaveCuratorImage,
        },
      },
    })

    const modelOptionsWithImage: CuratorModelProviderOption[] = [
      {
        id: "bailian",
        name: "Bailian",
        models: [
    ...
    const selectedModelWithImage: CuratorModelSelection = {
      provider: "bailian",
    ...
    const fileInput = screen
      .getByTestId("curator-input-container")
    ...
    // 校验 mockSaveCuratorImage 调用次数限制为 6（只保存了前 6 张）
    await waitFor(() => {
      expect(mockSaveCuratorImage).toHaveBeenCalledTimes(6)
    })
  ...
  it("输入 /session 时，能够渲染会话切换面板并通过关键字过滤和键盘切换会话", async () => {
    const mockSessions: CuratorSession[] = [
      {
        id: "session-foo",
        title: "Vue 对话",
        time: "10:00",
        messages: [{ id: "m1", role: "user", content: "test", time: "10:00" }],
        status: "completed",
      },
