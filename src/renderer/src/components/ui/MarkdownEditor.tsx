...
const getPositionedInlineDiffBlocks = (
  ...
  return blocks.flatMap((block) => {
    ...
    const to =
      originalLength === 0
        ? from
        : lineOffsets[index + originalLength - 1] + block.originalLines[originalLength - 1].length
...
const createInlineDiffExtension = () => {
  const inlineDiffField = StateField.define<InlineDiffState>({
    ...
    update: (value, transaction) => {
      ...
      const nonOverlappingDecorations = decorations.reduceRight<InlineDiffDecoration[]>(
        (result, decoration) => {
      ...
      const sortedDecorations = nonOverlappingDecorations.sort(
        (a, b) => a.from - b.from || a.decoration.startSide - b.decoration.startSide,
      )
