...
const findToolStepByPart = (
  ...
  part && part.kind === "tool" && "stepId" in part
...
export const PromptAiChatMessageBubble = ({
  ...
  const markdownContent = isUser
    ? message.content
    : messageParts
        .filter((part): part is Extract<PromptAiPart, { kind: "text" }> => part.kind === "text")
  ...
  const suggestedQuestionContextKey =
    suggestedQuestionContext?.map((item) => `${item.role}:${item.content}`).join("\u0000") ?? ""
  ...
  React.useEffect(() => {
    if (
      ...
      isGenerating ||
      message.cancelled ||
      !stableSuggestedQuestionContext.length ||
      !window.api?.ai?.suggestQuestions
    ) {
    ...
    void window.api.ai
      .suggestQuestions(stableSuggestedQuestionContext, suggestedQuestionHistoryRef.current)
      .then((questions) => {
        if (active) {
          const excludedQuestions = new Set(
            suggestedQuestionHistoryRef.current.map((question) => question.trim()),
          )
          const nextQuestions = questions.filter(
            (question) => !excludedQuestions.has(question.trim()),
          )
          suggestedQuestionHistoryRef.current = [
            ...suggestedQuestionHistoryRef.current,
            ...nextQuestions,
          ]
    ...
    message.cancelled,
    message.id,
    stableSuggestedQuestionContext,
    suggestedQuestionGenerationVersion,
  ])
  ...
  const renderAssistantParts = (): React.JSX.Element[] => {
    ...
    const renderExecutionParts = (
      ...
      groupExecutionParts(sequenceParts).flatMap((item, itemIndex, groups) => {
        if (item.kind === "text") {
          ...
          return part?.kind === "text" && part.content
            ? [
                <div
                  ...
                  className="markdown-preview-container curator-markdown-preview select-text max-w-full"
                >
                  <MdPreview
                    theme="dark"
                ...
                return (
                  <CuratorThinkingBlock
                    ...
                    isGenerating={
                      ...
                      (isGenerating && part.id === messageParts.at(-1)?.id)
                    }
                    connectsToNextExecution={nextExecution}
                  />
                )
              }
              ...
              return steps.length > 0 ? (
                <CuratorToolCallBlock
                  ...
                  connectsToNextExecution={
                    nextExecution || (itemIndex === groups.length - 1 && connectsToNextExecution)
                  }
                />
              ) : (
                <></>
    ...
    for (let index = 0; index < visibleMessageParts.length; index += 1) {
      ...
      if (step?.mcp) {
        ...
        renderedParts.push(
          <PromptAiMcpCallBlock
            key={`${message.id}-${part.id}`}
            steps={mcpSteps}
            connectsToNextExecution={nextPart?.kind === "tool" || nextPart?.kind === "reasoning"}
          />,
        )
        index = nextIndex - 1
        continue
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col gap-1 min-w-0 ${isNonTimedSystemMessage ? "w-full" : isUser ? "items-end" : "flex-1"}`}
      >
        <div
          className={`rounded-[6px] px-1 py-1 text-sm leading-relaxed break-words max-w-full ${isNonTimedSystemMessage ? "w-full" : "w-fit"} ${isUser ? "bg-transparent text-white font-medium whitespace-pre-wrap" : "text-white/80"} ${message.cancelled ? "line-through opacity-50" : ""}`}
        >
          {isUser ? (
            isEditing ? (
              <div className="flex w-[400px] max-w-full flex-col gap-2 rounded-[6px] border border-white/10 bg-[#212121] p-2.5">
                ...
                <div className="flex justify-end gap-1.5 border-t border-white/5 pt-2">
                  <button
                    type="button"
                    className="rounded-[4px] px-2 py-1 text-xs text-white/50 hover:bg-white/10"
                    onClick={() => {
                      setIsEditing(false)
                  ...
                  <button
                    type="button"
                    className="rounded-[4px] bg-white/10 px-2 py-1 text-xs text-white hover:bg-white/15 disabled:opacity-40"
                    disabled={
                      !editText.trim() || editText.trim() === message.content.trim() || isGenerating
                    }
                    onClick={() => {
                      setIsEditing(false)
                      void onEditAndResendUserMessage?.(message.id, editText.trim())
                  ...
                  <div className="flex flex-wrap justify-end gap-1">
                    {message.references.map((reference) => (
                      <Tooltip
                        key={reference.id}
                        content={
                          <pre className="max-h-60 max-w-[360px] overflow-auto whitespace-pre-wrap text-xs">
                            {reference.content}
                          </pre>
                        }
                      >
                        <Tag size="small" onClick={() => onReferenceSelect?.(reference)}>
            ...
            <div className="flex flex-col gap-1.5 max-w-full">
              {isSystem ? (
                <div className="px-1 text-xs font-mono uppercase text-white/40">
                  System Message · MCP Tools
                </div>
              ) : null}
              {message.mcpServers ? (
                <PromptAiMcpOverview servers={message.mcpServers} />
              ) : (
        ...
        {!isUser && suggestedQuestionContext ? (
          <SuggestedQuestions
            questions={suggestedQuestions}
            isLoading={isLoadingSuggestedQuestions}
            onSelect={(question) => onSendSuggestedQuestion?.(question)}
          />
        ) : null}
        {!isNonTimedSystemMessage ? (
          <div
            className={`text-xs font-mono mt-0.5 px-1 text-white/30 flex items-center gap-1.5 min-h-[1.25rem] ${isUser ? "justify-end text-right" : "justify-start text-left"}`}
          >
            <span>
              {isGenerating ? (
