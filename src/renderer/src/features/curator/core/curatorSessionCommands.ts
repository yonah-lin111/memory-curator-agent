...
// 会话状态派发函数。
type CuratorSessionDispatch = (action: CuratorSessionAction) => void

// Toast 提示端口。
type CuratorToastPort = {
  // 成功提示。
...
// 删除会话持久化端口。
type DeleteCuratorSessionPort = (sessionId: string) => Promise<void>

// 重新发送 AI 消息端口。历史重发只使用原始用户问题，不恢复旧 agent 选择。
type StartCuratorMessage = (
  text: string,
  sessionId: string,
  sourceSessions: CuratorSession[],
) => void
 ...
 */
export const renameCuratorSession = async ({
  sessionId,
  title,
  sessions,
  activeId,
  ...
  // 会话状态派发函数。
  dispatch: CuratorSessionDispatch
}): Promise<boolean> => {
 ...
 */
export const deleteCuratorSession = async ({
  sessionId,
  sessions,
  activeId,
  deleteSession,
  ...
  const fallbackSession = nextSessions.length === 0 ? createEmptyCuratorSession() : null
 ...
 */
const deleteCuratorSessionAndOpenDraft = async ({
  sessionId,
  sessions,
  deleteSession,
  clearSessionContext,
  ...
  sessions: CuratorSession[]
  // 持久化删除函数。
  deleteSession?: DeleteCuratorSessionPort
  // 清理上下文函数。
  clearSessionContext: (sessionId: string) => void
  // 会话状态派发函数。
  dispatch: CuratorSessionDispatch
  ...
  const existingDraft = remainingSessions.find(isEmptyCuratorDraftSession)
  const draftSession = existingDraft ?? createEmptyCuratorSession()
  const resolvedSessions = existingDraft
    ? [existingDraft, ...remainingSessions.filter((session) => session.id !== existingDraft.id)]
 ...
 */
export const undoLastCuratorTurn = async ({
  session,
  sessions,
  activeId,
  undoLastTurn,
  ...
  const nextSession = buildSessionWithMessages(session, session.messages.slice(0, turnStartIndex))

  if (!undoLastTurn) {
    removeRunMappingsByMessageIds(removedMessageIds)
    if (nextSession.messages.length === 0) {
      const isDeleted = await deleteCuratorSessionAndOpenDraft({
 ...
 */
export const deleteCuratorTurn = async ({
  messageId,
  session,
  sessions,
  activeId,
  ...
  const removedMessages = session.messages.slice(turnBounds.startIndex, turnBounds.endIndex)
  const removedMessageIds = new Set(removedMessages.map((message) => message.id))
 ...
 */
export const regenerateLatestCuratorAnswer = async ({
  session,
  sessions,
  activeId,
  undoLastTurn,
  ...
  removeRunMappingsByMessageIds: RemoveRunMappingsByMessageIds
  // 重新发送消息函数。
  startCuratorMessage: StartCuratorMessage
  // 会话状态派发函数。
  dispatch: CuratorSessionDispatch
  // Toast 提示端口。
  toast: CuratorToastPort
  ...
  try {
    ...
    const cleanSessions = sessions.map((item) => (item.id === session.id ? cleanSession : item))

    removeRunMappingsByMessageIds(removedMessageIds)
    dispatch({ type: "reset", sessions: cleanSessions, activeId })
    startCuratorMessage(userMessage.content, session.id, cleanSessions)
