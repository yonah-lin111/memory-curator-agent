...
import { CuratorInput, type CuratorInputCommandId } from '@/features/curator/components/CuratorInput'
...
const renderCuratorInput = (
  onCommandExecute: (command: CuratorInputCommandId) => string | void | Promise<string | void> = () => undefined,
  isGenerating = false,
...
describe('CuratorInput', () => {
  ...
  it('当选择的模型支持图片输入时，最多一次性上传 6 张图片，超出则警告并截断', async () => {
    ...
    // 查找并获取 file input
    const fileInput = screen.getByTestId('curator-input-container').querySelector('input[type="file"]') as HTMLInputElement
