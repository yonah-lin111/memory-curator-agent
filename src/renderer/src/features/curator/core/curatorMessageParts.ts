import type { CuratorMessagePart, CuratorSession } from "@/features/curator/types"
 ...
 */
export const completeAiMessageReasoningParts = (message: CuratorMessage): CuratorMessage => {
  ...
  return {
    ...
    parts: message.parts.map((part) =>
      part.kind === "reasoning" && part.status !== "done" ? { ...part, status: "done" } : part,
    ),
  }
}
 ...
 */
export const appendAiMessageTextPart = (message: CuratorMessage, chunk: string): CuratorMessage => {
...
export const appendAiMessageReasoningPart = (
  ...
  const isContinuingLastReasoning =
    lastPart?.kind === "reasoning" && resolveReasoningSourceId(lastPart) === reasoningId
...
export const appendAiMessageToolPart = (
  ...
  if (completedMessage.parts?.some((part) => part.kind === "tool" && part.stepId === stepId)) {
