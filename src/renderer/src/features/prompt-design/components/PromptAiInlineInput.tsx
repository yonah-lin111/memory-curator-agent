...
export const PromptAiInlineInput = ({
  ...
  useLayoutEffect(() => {
    ...
    // 计算面板应该向上还是向下展开
    // 计算输入框渲染后的实际上下可用空间
    const inputActualTop =
      renderTop !== "auto" ? renderTop : window.innerHeight - (renderBottom as number) - inputHeight
    const inputActualBottom =
      renderTop !== "auto" ? renderTop + inputHeight : window.innerHeight - (renderBottom as number)
        ...
        <div className="flex flex-wrap gap-1 px-1">
          {controller.references.map((reference) => (
            <Tag
              ...
              size="small"
              onClose={() =>
                controller.setReferences((items) =>
