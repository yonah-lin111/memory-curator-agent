...
import type { CuratorEvent, CuratorSession, CuratorStartPayload } from "@/features/curator/types"
...
describe("App", () => {
  ...
  it("支持从左侧栏切换到其他静态页面", async () => {
    ...
    expect(
      within(screen.getByLabelText("Sidebar main navigation")).getByRole("button", {
        name: /Journal/,
      }),
    ...
    expect(screen.getByRole("button", { name: /Weekly Review/ })).toHaveAttribute(
      "aria-current",
      "page",
    )
  ...
  it("支持删除待办项", async () => {
    ...
    const deleteBtn = container.querySelector(
      'button[aria-label="Delete todo 完成 Today 工作台的三栏静态布局编码与视觉自审"]',
    )!
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("完成 Today 工作台的三栏静态布局编码与视觉自审"),
  ...
  it("点击自由随记卡片添加按钮后打开对应弹窗", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "第 1 条待办选择优先级P0" }),
    ).not.toBeInTheDocument()
  })
  ...
  it("Header 聊天按钮左侧展示上下文记录 tooltip 按钮和工具策略详情", async () => {
    ...
    expect(
      contextButton.compareDocumentPosition(chatButton) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy()
  ...
  it("首次加载持久化历史摘要后点击新建对话直接创建空白会话", async () => {
    ...
    expect(
      within(historyList).getByText("新建对话").closest('[aria-current="true"]'),
    ).toBeInTheDocument()
  })
})
