...
import { MAX_TEXT_FILES } from "@/lib/ai-shared/constants";
import { isTextFile } from "@/lib/ai-shared/utils";

/**
 * useCuratorFiles - 专门管理 AI 输入框关联图片及文本文件选择、上传、拖拽和粘贴行为的微 Hook。
 *
 * @param isImageSupported 当前 AI 模型是否支持图片上传
 */
export const useCuratorFiles = (isImageSupported: boolean) => {
  ...
  const handleUploadFiles = async (files: FileList | File[]): Promise<void> => {
    ...
    if (!window.api?.files?.saveCuratorImage) {
    ...
    for (const file of allowedFiles) {
      ...
      try {
        ...
        const result = await window.api.files.saveCuratorImage({
          name: file.name,
          mimeType: file.type,
          bytes: buffer,
        });
  ...
  const handleUploadTextFiles = async (
    ...
    if (!window.api?.files?.saveCuratorTextFile) {
    ...
    for (const file of allowedFiles) {
      ...
      try {
        const buffer = await file.arrayBuffer();
        const result = await window.api.files.saveCuratorTextFile({
          name: file.name,
          mimeType: file.type,
          bytes: buffer,
        });
