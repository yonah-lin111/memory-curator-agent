...
import type {
  CuratorContextBudget,
  CuratorContextItem,
} from "@/features/curator/curatorContextBuilder";

// AI 对话上下文时间线组件属性类型。
type CuratorContextTimelineProps = {
  // 当前会话上下文条目。
  items: CuratorContextItem[];
  // 当前上下文预算。
  budget: CuratorContextBudget;
  // 当前会话的原始消息列表。
...
type QaTurn = {
  ...
  id: string;
  // 用户提问消息上下文（可选）。
  userMessage?: CuratorContextItem;
  // 助手回答消息上下文（可选）。
  assistantMessage?: CuratorContextItem;
  // 此回合调用的工具。
  tools: CuratorContextItem[];
  // 此回合注入的辅助上下文。
  injectedContexts: CuratorContextItem[];
  // 总估算 token 数。
  tokens: number;
  // 创建时间。
  createdAt: number;
 ...
 * 将平铺的上下文条目聚合为 QA 回合。
 */
const groupItemsByQaTurn = (items: CuratorContextItem[]): QaTurn[] => {
  const sorted = [...items].sort((a, b) => a.createdAt - b.createdAt);
  const turns: QaTurn[] = [];
  let currentTurn: QaTurn | null = null;
  let pendingInjected: CuratorContextItem[] = [];
...
/**
 * CuratorContextTimeline - 以两个聊天列表形式展示在消息列表右侧的上下文调试时间线。
 */
export const CuratorContextTimeline = ({
  items,
  budget,
  messages = [],
  isOpen = false,
  isTransitionEnabled = true,
}: CuratorContextTimelineProps): React.JSX.Element => {
  ...
  // 仅在未选择任何文本（准备复制）时触发展开/折叠
  const handleToggleNodeWithSelectionCheck = (nodeKey: string) => {
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 text-white/50">
                ...
                <span className="text-white/20">
                  ...
                  {budget.contextLimit !== undefined
                    ? ` / ${budget.contextLimit.toLocaleString("zh-CN")}`
                    : ""}{" "}
                  tokens)
                </span>
              </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {turns.length > 0 && (
                <IconButton
                  ...
                  aria-label={
                    isAllExpanded ? "Collapse all turns" : "Expand all turns"
                  }
                  size="small"
                >
                  {isAllExpanded ? (
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar [scrollbar-gutter:stable] flex flex-col gap-2 pr-1">
              {turns.map((turn, index) => {
                ...
                const fullMessage = messages.find(
                  (m) => m.id === turn.assistantMessage?.sourceId,
                );

                return (
                  <div
                    key={turn.id}
                    className="rounded-[6px] bg-white/[0.02] border border-white/[0.03]"
                  >
                    {/* 折叠头部 */}
                    <div
                      ...
                      <div className="mx-3 mb-2.5 pl-3 border-l border-white/5 flex flex-col gap-3 relative mt-1 select-text">
                        ...
                        {turn.userMessage &&
                          (() => {
                            ...
                            const fullUserMsg = messages.find(
                              (m) => m.id === userMsgKey,
                            );
                            ...
                            return (
                              <div className="relative pl-3 select-text">
                                ...
                                <div
                                  onClick={() =>
                                    handleToggleNodeWithSelectionCheck(
                                      turn.userMessage!.key,
                                    )
                                  }
                                ...
                                >
                                  ...
                                  {turn.userMessage.content.length >
                                    turn.userMessage.summary.length && (
                          ...
                          <div className="relative pl-3">
                            ...
                            <div className="mt-1 flex flex-col gap-1.5 select-text">
                              {turn.injectedContexts.map((ctx) => (
                                <div
                                  key={ctx.key}
                                  className="bg-white/[0.01] p-1.5 rounded-[4px] border border-white/[0.02]"
                                >
                        ...
                        {(() => {
                          ...
                          if (
                            fullMessage?.parts &&
                            fullMessage.parts.length > 0
                          ) {
                            fullMessage.parts.forEach((part, partIndex) => {
                              if (part.kind === "reasoning") {
                                nodes.push(
                                  <div
                                    key={`${turn.id}-reasoning-${partIndex}`}
                                ...
                                const tool = turn.tools.find(
                                  (t) => t.sourceId === part.stepId,
                                );
                                if (tool) {
                                    ...
                                    nodes.push(
                                      ...
                                      >
                                        ...
                                        <div className="flex items-center gap-1.5 text-[11px] text-amber-400/70 font-mono select-none">
                                          ...
                                          <span className="text-[10px] text-amber-500/60 font-sans select-none">
                                            ({step.status})
                                          </span>
                                        </div>
                        ...
                        {turn.assistantMessage && (
                          <div className="relative pl-3 select-text">
                            ...
                            >
                              ...
                              {turn.assistantMessage.content.length >
                                turn.assistantMessage.summary.length && (
                                <span className="text-white/30 text-[10px] ml-1 select-none hover:underline">
                                  {expandedNodes[turn.assistantMessage.key]
                                    ? "[折叠]"
                                    : "...[展开]"}
