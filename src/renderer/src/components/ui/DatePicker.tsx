...
export const DatePicker = ({
  ...
  const [visibleMonth, setVisibleMonth] = useState<string>(() =>
    value && mode !== "month" ? getEntryMonth(value) : getEntryMonth(createTodayEntryDate()),
  )
  // 控制月度选择器中的年份。
  ...
  const updatePosition = (): void => {
    if (isOpen && tooltipRef.current && containerRef.current) {
      ...
      // 获取当前气泡主体的高度和宽度
      const tooltipHeight = tooltipRef.current.offsetHeight || tooltipRect.height || 320
      const tooltipWidth = tooltipRef.current.offsetWidth || tooltipRect.width || 296
              ...
              <>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-white/86">{visibleYear}</p>
                  </div>
              ...
              <>
                ...
                <div className="mt-3 grid grid-cols-7 gap-1 text-center">
                  {CALENDAR_WEEKDAY_LABELS.map((weekdayLabel) => (
                    <span key={weekdayLabel} className="text-[10px] font-mono text-white/28">
                      {weekdayLabel}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-2 grid grid-cols-7 gap-1">
                  {calendarDays.map((dayItem) => {
                    ...
                    const isSelected =
                      mode === "week"
                        ? value && getMonday(dayItem.entryDate) === getMonday(value)
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          onChange(
                            mode === "week" ? getMonday(dayItem.entryDate) : dayItem.entryDate,
                          )
                        }}
                      >
