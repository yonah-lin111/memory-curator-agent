...
export const formatAskAnswerObservation = (data: AskAnswerData): string => {
  const formatted = data.answers
    .map(
      (item) =>
        `"${item.question}"="${item.answers.length > 0 ? item.answers.join(", ") : "Unanswered"}"`,
    )
    .join(", ")
 ...
 * 从请求和二维答案构造 Ask 回答数据。
 */
export const createAskAnswerData = (
...
export const createAskTool = (): AgentTool => ({
  ...
  description:
    "Ask the user structured clarification questions when critical information is missing.",
  prompt: {
    summary:
      "Ask the user structured clarification questions when critical information is missing.",
