 ...
 */
export const groupExecutionParts = (parts: ExecutionSequencePart[]): ExecutionSequenceGroup[] => {
...
type ExecutionGroupBlockProps = {
  ...
  // 渲染单个执行片段。
  renderPart: (
    part: ExecutionGroup["parts"][number],
    connectsToNextExecution: boolean,
  ) => React.JSX.Element
  // 渲染连续的工具片段。
  renderToolParts: (
...
export const ExecutionGroupBlock = ({
  ...
  if (!isCollapsible) {
    return <div className="flex flex-col gap-1.5 w-full">{renderGroupedParts()}</div>
  }
