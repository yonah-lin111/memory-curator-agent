...
import { BookOpen, CalendarDays, Clock, Tag, User, Sparkles, AlertCircle } from 'lucide-react'
...
const JOURNAL_ENTRIES: JournalEntryItem[] = [
  {
    ...
    weekday: 'Mon',
    time: '21:45',
    content: '今天上海又下了小雨。在写完了 Today 工作台的布局后，看着黑色的背景板 and 克制的白色边框，有一种异样的平静。人脑里的记忆其实也是这样，乱七八糟，而我们需要一个外在的、数字化的“海马体”来帮我们整理。我把那些写在碎纸片和微信文件传输助手里的垃圾信息全都归档了，只留下了最重要的几条。今晚不需要焦虑，明天继续推进 AEON 神经关联图谱的设计。希望这个界面能够作为我新生活的见证者。',
    mood: '平静 / 专注',
    people: ['自我', 'Yonah'],
    themes: ['数字海马体', '设计系统'],
    historicalConnection: '本条目中的“微雨平静”与 3 天前 (05-22) 提到的“精疲力竭”形成情绪对冲，系统检测到你在雨天的专注度显著提升。'
  },
  {
    id: 'j-2',
    date: '2026-05-24',
    weekday: 'Sun',
    time: '16:30',
    content: '周末在安福路的咖啡馆坐了很久，看着人来人往，突然有些失落。现在的社交媒体无时无刻不在用算法投喂垃圾，把每个人的瞬间记忆切碎成以秒计算的短视频。我们在主动向算法让渡我们的记忆主权。如果有一天我们的回忆全是由字节跳动的推荐列表构成的，那我们还是我们自己吗？本地优先、纯粹归属个人的 Memory Curator 或许是唯一的解药。我要把我所有的脆弱和零碎想法都留在本地。',
    mood: '失落 / 警醒',
    people: ['自我'],
    themes: ['个人边界', '数字海马体'],
    historicalConnection: '这篇关于“记忆主权让渡”的思索，与 05-20 笔记中的「AI协同与个人边界」观点存在 89% 的思想契合，已自动聚合为主题“数字海马体”的底层支柱。'
  },
  {
    id: 'j-3',
    date: '2026-05-22',
    weekday: 'Fri',
    time: '23:10',
    content: '一整天都在和 Rust 的桥接层编译报错作斗争。由于多维关联图谱需要极致的性能，我固执地拒绝了现成的客户端 SQL 图谱，非要用本地二进制碎块进行神经关联度衰减模型计算。编译器一遍遍报错，中途差点想放弃退回轻量 SQLite。在晚上九点终于编译通过的那一刻，看着 15ms 的渲染耗时，我知道我的固执是值得的。极度疲惫，但是内心感觉被填满了。',
    mood: '精疲力竭 / 充实',
    people: ['自我'],
    themes: ['本地优先架构', '性能攻坚'],
    historicalConnection: '本篇中“拒绝妥协”的技术偏执，与上周五 (05-15) 在笔记里记录的“架构第一性原理”高度重合。你的行为模式显示：你在面对核心性能瓶颈时倾向于选择硬核自研。'
  },
  {
    id: 'j-4',
    date: '2026-05-19',
    weekday: 'Tue',
    time: '19:05',
    content: '今天本应该着手重构多维图谱冷启动的懒加载逻辑，但我却把一下午的时间花在修一个无关紧要的 Lint 报错上。事后回想，这是一种非常典型的“创造性拖延”——因为对重构冷启动的底层逻辑感到有些无从下手，所以潜意识里选择了用解决轻量错误来获取虚假的成就感。我不能再这样逃避核心问题。需要让 Agent 在未来的计划区中对此进行高亮警示。',
    mood: '焦虑 / 自省',
    people: ['自我', 'Yonah'],
    themes: ['计划延后模式', '拖延机制'],
    historicalConnection: '本条目的“逃避心理自省”是你在 5 月份第 4 次记录到“创造性拖延”。系统已把这一重复模式捕获到长期追踪主题「计划延后模式」中。'
  }
]
  ...
  return (
    ...
    >
      ...
      <header className="flex flex-col gap-1 flex-shrink-0">
        ...
        <h1 className="text-lg font-bold tracking-tight text-white">历史日记条目</h1>
      </header>
        ...
        <div className="flex-1 rounded-[6px] border border-white/5 bg-[#212121] p-5 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
          ...
          <div className="border-b border-white/5 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-2.5 text-xs text-white/40 mt-1">
                <span className="font-bold text-white/70">{activeEntry.date}</span>
                <span>•</span>
                <span>星期{activeEntry.weekday === 'Mon' ? '一' : activeEntry.weekday === 'Sun' ? '日' : activeEntry.weekday === 'Fri' ? '五' : '二'}</span>
                <span>•</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-1">
            ...
            <div className="rounded-[6px] border border-white/5 bg-white/[0.01] p-3 flex flex-col gap-2">
              <h3 className="text-xs font-bold text-white/75">提及人物 & 关联线索</h3>
