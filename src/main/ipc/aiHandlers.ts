...
const SYSTEM_PROMPT_SECTIONS = [
  ...
  '提问边界：当缺少关键范围、偏好或选择且猜测会导致返工时，使用 common_tool.ask 向用户提出一到三个结构化问题；能基于现有上下文保守推进时不要提问。',
  'People 写入边界：common_tool.ask 不能用于确认人物档案添加、修改或删除；需要写入时先用 common_tool.explain 说明，再直接调用对应 people_tool，系统会处理内部确认。',
