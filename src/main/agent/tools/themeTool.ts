...
export const createThemeTools = (themesService: ThemesService): AgentTool[] => [
  {
    ...
    description:
      "查询当前所有主题及其关联，支持 SQL 查询。列出所有主题、某主题的所有关联素材、或执行自定义只读 SQL。",
    prompt: {
    ...
    parameters: {
      ...
      properties: {
        action: {
          ...
          description:
            "操作类型：list_themes 列出所有主题，list_items 列出某主题的关联，sql 执行自定义只读查询",
        },
        themeExternalId: {
          type: "string",
  ...
  },
  {
    ...
    confirmation: {
      ...
      renderSummary: (input: any) =>
        `将更新主题 ${input?.themeExternalId}（新名称：${input?.name || "未修改"}）`,
    },
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    confirmation: {
      ...
      renderSummary: (input: any) =>
        `将 ${input?.sourceType} ${input?.sourceId} 关联至主题 ${input?.themeExternalId}`,
    },
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    confirmation: {
      ...
      renderTarget: (input: any) => `素材 ${input?.sourceId}`,
      renderSummary: (input: any) =>
        `将 ${input?.sourceType} ${input?.sourceId} 与主题 ${input?.themeExternalId} 解除关联`,
    },
    parameters: {
      type: "object",
      properties: {
