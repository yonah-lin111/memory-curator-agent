...
describe("promptDesignService", () => {
  ...
  it("correctly updates prompt design items", () => {
    ...
    const designRow = testDb
      .prepare("SELECT * FROM prompt_design_items WHERE external_id = 'd-1'")
      .get() as any
    ...
    expect(() =>
      testDb
        .prepare("UPDATE prompt_design_items SET status = 'invalid' WHERE external_id = 'd-1'")
        .run(),
    ).toThrow()
  })
