...
const prepareJournalSql = (sql: string, limit: number): string => {
  ...
  if (normalizedSql.includes(';') || SQL_COMMENT_PATTERN.test(normalizedSql)) {
    throw new Error(
      'Journal SQL only allows a single SELECT statement without comments'
    )
  }
  ...
  if (
    !new RegExp(`\\bfrom\\s+${JOURNAL_TABLE_NAME}\\b`, 'i').test(normalizedSql)
  ) {
...
const queryBySql = (
  ...
  const rows = journalsService
    .querySql(prepareJournalSql(sql, limit))
    .slice(0, limit)
...
export const createJournalQueryTool = (
  ...
  description:
    'Query journals in the local Journals table. Read-only; never modifies data.',
  prompt: {
    summary:
    ...
    output:
      'Return the journal facts needed to answer the user. Do not repeat irrelevant fields.',
    examples: [
      `{"limit":10}`,
      `{"query":"工作总结"}`,
      `{"datePrefix":"2026-06"}`,
  ...
  execute: async (input) => {
    ...
    const limit = Math.max(
      1,
      Math.min(parsed.limit ?? DEFAULT_JOURNAL_LIMIT, MAX_JOURNAL_LIMIT)
    )
 ...
 * 构建 Journal 写入确认配置。
 */
const buildJournalConfirmation = (
  action: JournalWriteAction
...
const buildJournalWritePrompt = (action: JournalWriteAction) => {
  ...
  return {
    summary:
      `${actionVerb} a journal entry in the local Journals table.`,
    intentKeywords:
      action === 'delete'
    ...
    output:
      'Confirm the action result to the user.'
  }
}
...
export const createJournalDeleteTool = (
  ...
  description:
    'Delete an existing journal entry by date.',
  ...
  parameters: {
    ...
    properties: {
      confirmationSummary: {
        type: 'string',
        description:
          'Concise Markdown Chinese explanation shown above the internal confirmation.'
      },
      entryDate: {
        type: 'string',
        description: 'Journal date in YYYY-MM-DD format'
...
const renderJournalBatchMutationSummary = (
  ...
  const count = input.entries.length
  const actionLabel =
    action === 'delete' ? '删除' : action === 'update' ? '更新' : '新建'
 ...
 * 构建批量 Journal 确认配置。
 */
const buildJournalBatchConfirmation = (
  action: JournalWriteAction
): ToolConfirmationConfig => ({
  header: '确认批量操作',
  ...
  completion: {
    renderMessage: (_input, _result) =>
      action === 'delete'
        ? 'Batch journal entries deleted.'
        : 'Batch journal entries saved.'
  }
})
