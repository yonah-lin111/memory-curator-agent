...
import { type AskAnswerPayload, submitAskAnswer } from "@/ipc/ai/ask"
import {
  cancelAiChatAsk,
...
export function registerPromptAiHandlers(): void {
  ...
  ipcMain.handle(
    "prompt-ai:mcp:status",
    async (_, payload: { designItemId: string }): Promise<PromptAiMcpStatusResult> => {
  ...
  ipcMain.handle(
    ...
    async (
      _,
      payload: { sessionId: string; designItemId: string },
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, payload: PromptAiChatStartPayload) => {
      ...
      // 启动后台进程
      Promise.resolve()
        .then(() => runPromptAiChat(event.sender, payload, runId, abortController.signal))
        .catch(console.error)
  ...
  ipcMain.handle(
    ...
    (_, payload: { runId: string; designItemId: string; content: string; version: number }) => {
      ...
      if (
        !run?.readDocument ||
        run.designItemId !== payload.designItemId ||
  ...
  ipcMain.handle("prompt-ai:chat:ask-answer", (_, payload: AskAnswerPayload) =>
...
async function runPromptAiChat(
  ...
  const shouldCreateTitle = !existingSession || existingSession.messages.length === 0
  ...
  // 解析项目路径，创建文件工具集
  const projectRoot = promptDesignService.getProjectPathByDesignItemId(payload.designItemId)
  const readDocument = (): Promise<PromptEditorDocument> =>
  ...
  const applyDocument = (
    ...
    new Promise((resolve, reject) => {
      ...
      run.rejectPendingEditorRequest = rejectRequest
      sender.send("prompt-ai:editor:apply", {
        runId,
        designItemId: payload.designItemId,
    ...
    for (const [index, m] of session.messages.entries()) {
      ...
      if (role === "assistant" && m.toolSteps && m.toolSteps.length > 0) {
        for (const step of m.toolSteps) {
          ...
          let content = ""
          if (step.status === "done") {
            content = renderToolResultContent(step.observation || "Success", step.data)
  ...
  const systemMessage: AgentMessage = {
    role: "system",
    content: loadPromptDesignAgentPrompt(),
  }
  ...
  if (availableSkills.length > 0) {
    systemMessage.content += `\n\n  <available-skills>\n    <instruction>当技能名称与用户请求匹配时，使用 load_skill 加载其完整指令。</instruction>\n${availableSkills
      .map(
        (skill) =>
          `    <skill><name>${escapeXmlText(skill.id)}</name><description>${escapeXmlText(skill.description || skill.name)}</description></skill>`,
      )
      .join("\n")}\n  </available-skills>`
  }
