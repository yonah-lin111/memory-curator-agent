...
import { type ExecutionGroup, ExecutionGroupBlock } from "@/components/ai-shared/ExecutionGroup"
...
describe("ExecutionGroupBlock", () => {
  ...
  it("将连续工具片段批量交给工具渲染器", () => {
    const renderToolParts = vi.fn(
      (toolParts: Extract<ExecutionGroup["parts"][number], { kind: "tool" }>[]) => (
        <div key={toolParts[0].id}>{toolParts.map((part) => part.id).join(",")}</div>
      ),
    )
    ...
    expect(combinedReadParts?.[0].map((part) => part.id)).toEqual(["read-1", "read-2"])
  })
})
