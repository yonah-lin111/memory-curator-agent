...
export const createPeopleBatchUpdateTool = (
  ...
  prompt: {
    ...
    output:
      "Include confirmationSummary in the tool arguments; return count and updated profile facts needed by the user.",
    examples: [
  ...
  execute: async (input) => {
    ...
    const results = (input.items as unknown[]).map((item) => {
      ...
      const updated = peopleService.update(
        id,
        parsePersonProfileInput(item as Record<string, unknown>),
      )
