...
} from "@/features/curator/core/curatorReasoningDedupe"
...
// AI 聊天气泡组件属性类型。
type CuratorMessageBubbleProps = {
  // 当前消息数据。
  ...
  // 打开消息右键菜单回调。
  onOpenContextMenu: (request: CuratorMessageContextMenuRequest) => void
  // 编辑并重新发送用户消息。
...
// AI 消息右键菜单打开请求类型。
export type CuratorMessageContextMenuRequest = {
  // 菜单所属消息 ID。
  messageId: string
  // 菜单显示横坐标。
  x: number
...
// Markdown 预览组件属性类型。
type CuratorMarkdownPreviewProps = {
  // Markdown 正文。
  content: string
  // 附加容器类名。
  className?: string
...
/**
 * CuratorMarkdownPreview - 渲染 AI 内容 Markdown 片段。
 */
const CuratorMarkdownPreview = ({
  content,
  className = "",
  isGenerating = false,
}: CuratorMarkdownPreviewProps): React.JSX.Element => {
  ...
  return (
    <div
      ref={containerRef}
      className={`markdown-preview-container curator-markdown-preview select-text max-w-full ${className}`}
    >
      <MdPreview
        theme="dark"
        modelValue={content}
...
const resolveAiMessageParts = ({
  ...
  isProcessing,
  displayAnswer,
}: ResolveAiMessagePartsParams): CuratorMessagePart[] => {
  if (message.parts?.length) {
    return message.parts
  }

  const fallbackParts: CuratorMessagePart[] = []
 ...
 */
export const CuratorMessageBubble = ({
  message,
  isLastUser = false,
  isGenerating = false,
  canRegenerate = false,
  ...
  const isProcessing =
    !isUser &&
    (message.content.startsWith("Processing:") || message.content.startsWith("正在处理："))
  ...
  const dedupedRenderablePartContents = resolveDedupedRenderablePartContents(messageParts)
  const markdownContent = isUser ? message.content : dedupedTextContents.join("\n\n").trim()
  ...
  return (
    ...
    >
      {/* 消息气泡主体 */}
      <div className={`flex flex-col gap-1 min-w-0 ${isUser ? "items-end" : ""}`}>
        <div
                ...
                return groupExecutionParts(sequenceParts).flatMap((item) => {
                  if (item.kind === "text") {
                    ...
                    const displayText = part
                      ? (dedupedRenderablePartContents[messageParts.indexOf(part)] ?? "")
                      : ""
                    return displayText
                      ? [
                    ...
                    <ExecutionGroupBlock
                      ...
                      renderPart={(sequencePart, connectsToNextExecution) => {
                        const partIndex = messageParts.findIndex(
                          (candidate) => candidate.id === sequencePart.id,
                        )
                        ...
                        if (part?.kind === "reasoning") {
                          ...
                          return displayReasoning ? (
                            <CuratorThinkingBlock
                              ...
                              onToggle={onThinkingBlockToggle}
                              connectsToNextExecution={connectsToNextExecution}
                            />
                          ) : (
                            <></>
                          )
                      ...
                      renderToolParts={(toolParts, connectsToNextExecution) => {
                        const steps = toolParts.flatMap((toolPart) => {
                          const part = messageParts.find(
                            (candidate) => candidate.id === toolPart.id,
                          )
                          const step = findToolStepByPart(message.toolSteps, part)
                          return step ? [step] : []
                        })
        ...
        {/* 提及的 Agent 行 */}
        {isUser && message.parts && message.parts.some((p) => p.kind === "agent") && (
