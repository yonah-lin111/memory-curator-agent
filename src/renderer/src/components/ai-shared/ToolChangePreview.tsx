...
export type CuratorToolChangePreviewProps = {
  toolName: string
  input: unknown
      ...
      <div className="flex flex-wrap gap-1">
        {value.map((v, i) => (
          <Tag
            ...
            size="small"
            color="default"
            bgClass="border-white/10 bg-white/5 text-white/60"
          >
            {String(v)}
          </Tag>
        ))}
...
export const CuratorToolChangePreview = ({
  toolName,
  input,
  isGenerating = false,
}: CuratorToolChangePreviewProps): React.JSX.Element | null => {
  ...
  const getOperationBadge = (type: OperationType) => {
    switch (type) {
      ...
      case "batch_add":
        return (
          <span className="rounded-[4px] px-1.5 py-0.5 text-[11px] font-medium bg-emerald-500/10 text-emerald-400">
            新增
          </span>
        )
      case "update":
      case "batch_update":
        return (
          <span className="rounded-[4px] px-1.5 py-0.5 text-[11px] font-medium bg-amber-500/10 text-amber-400">
            编辑
          </span>
        )
      case "delete":
      case "batch_delete":
        return (
          <span className="rounded-[4px] px-1.5 py-0.5 text-[11px] font-medium bg-red-500/10 text-red-400">
            删除
          </span>
        )
      default:
  ...
  const renderSingleItem = (itemInput: Record<string, unknown>) => {
    const fields = Object.entries(itemInput).filter(
      ([k, v]) => k !== "confirmationSummary" && v !== null && v !== undefined && v !== "",
    )
        ...
        {topFields.map(([key, val]) => (
          <div key={key} className="flex items-start gap-2">
            ...
            <div className="text-[12px] text-white/60 leading-relaxed break-all min-w-0 flex-1">
              {key === "id" ? (
                <span className="font-mono text-white/50">{String(val)}</span>
              ) : (
                formatValue(key, val)
              )}
            </div>
    ...
    return (
      <div className="flex flex-col">
        {items.slice(0, 10).map((item, idx) => (
          <div
            key={idx}
            className="flex flex-col gap-1 py-2 border-b border-white/[0.05] last:border-b-0 last:pb-0 first:pt-0"
          >
  ...
  if (isRecord(input)) {
    ...
    content = (
      <pre className="text-[11px] text-white/45 font-mono overflow-x-auto">
  ...
  return (
    <div className="flex flex-col min-w-0">
      ...
      >
        ...
        <div
          className="flex items-center gap-1.5 flex-1 min-w-0 mt-[1px]"
          ...
          {summaryText ? (
            ...
            <>
              ...
              {operationType.startsWith("batch_") && batchCount > 0 && (
                <span className="text-[11px] text-white/45 bg-white/5 px-1.5 py-0.5 rounded-[4px]">
                  {batchCount} 项
                </span>
              )}
            </>
