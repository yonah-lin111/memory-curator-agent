...
const TRACKED_THEMES: ThemeItem[] = [
  {
    ...
    recordCount: 12,
    summary: '探索记忆的外在工具如何从“传统堆积记事”演化为“智能海马体”。核心在于引入轻量主动策展与退避遗忘机制，只在本地保留最高信噪比的主观核心神经元与事实。',
    relationshipSummary: '该主题的演化与你在雨天的平静度（呈显著正相关）以及对社交平台推荐算法的抵触情绪深度交织。在行为上，它主要由你对杂乱聊天记录的主动归档动作触发。',
    moods: ['平静 / 专注', '失落 / 警醒'],
    milestones: [
      {
        date: '2026-05-25',
        ...
        source: '深度日记',
        description: '成功将“快速随手捕获、每日待办、情绪日记”融合入经典黑色客户端，标志着数字海马体从纯概念探讨进入第一个可用的原型测试阶段。'
      },
      {
        date: '2026-05-24',
        title: '安福路午后：算法侵蚀与记忆主权反思',
        source: '深度日记',
        description: '探讨推荐算法如何无意识切碎和接管个人的深度思考与回忆。明确了 Memory Curator 建立完全离线的脑电波映射对于维护思维主权的核心哲学定位。'
      },
      {
        date: '2026-05-20',
        title: '微信群关于 AI 协同个人边界讨论',
        source: '随手随记',
        description: '记录了对“AI 快速输出 vs 保护个人注意力”的辩证思考。确立了必须在本地沙盒建立策展壁垒、不向公共大模型倾倒原始心智碎片的架构原则。'
      },
      {
        date: '2026-05-19',
        title: '数字海马体主动过滤与遗忘设想',
        source: '随手随记',
        description: '在读完海马体生理机制文章后，顿悟：信息过载的解药不是更强的搜索引擎，而是克制、主动的遗忘算法。所有的临时闪念都不应自动成为长期记忆。'
      }
    ]
  },
  {
    ...
    recordCount: 18,
    summary: '构建完全局限于本地 Keychain 和沙盒的加密存取方案。通过在本地二进制分片进行高并发的神经关联度衰减图谱计算，保障极致的低延迟体验和绝对的个人数据隐私。',
    relationshipSummary: '该主题多在技术攻坚阶段浮现，通常伴随极高能量的主观心境。在行为上，你极度抗拒平庸通用的第三方云服务托管数据库，倾向于自研硬核、第一性原理的存取机制。',
    moods: ['精疲力竭 / 充实', '专注'],
    milestones: [
      {
        date: '2026-05-25',
        ...
        source: '深度日记',
        description: '今天成功调试通过本地冷启动数据分片懒加载，将 Local-first 状态稳定运行于客户端底层，实现了 120ms 的冷启动无缝载入。'
      },
      {
        date: '2026-05-22',
        title: 'Rust 多维二进制解密首通',
        source: '深度日记',
        description: '在连续报错数小时后，成功用 Rust 编译通过了高并发的本地碎块解密与关联图谱衰减模型，多维图谱首次首屏渲染耗时被压低至硬核的 15ms。'
      },
      {
        date: '2026-05-19',
        title: '异步分片后台懒加载预处理决策',
        source: '会议记录',
        description: '为了解决冷启动时因多维神经图谱庞大而导致的卡顿，与 Yonah 决策：App 启动只初始化 Today 面板，将大体积关联网格全部交给后台子线程异步懒加载。'
      },
      {
        date: '2026-05-15',
        title: '确立“架构第一性原理”离线方案',
        source: '随手随记',
        description: '否定了传统 Web 云数据库加本地缓存的妥协方案。决定底层数据只存放于本地沙盒文件碎块，密钥直接由硬件级 Keychain 保护。云备份必须采用完全非对称加密碎碎化存储。'
      }
    ]
  },
  {
    ...
    recordCount: 7,
    summary: '深入剖析自身在开发高难度架构设计或复杂渲染系统时，潜意识里用“解决微小、重复的无意义技术 Lint 报错或界面微调”来获取虚假成就感、从而逃避核心痛点的“创造性拖延模式”。',
    relationshipSummary: '该主题主要伴随高强度的焦虑和烦躁。它的触发条件十分精准：只要当天的待办列表中存在需要耗费长考的、无先例可循的高难度架构任务，这一主题的记录就会在当晚日记中飙升。',
    moods: ['焦虑 / 自省'],
    milestones: [
      {
        date: '2026-05-25',
        ...
        source: '深度日记',
        description: '本周策展复盘中，Agent 自动将你在 21 日因为修整 Lint 而将 UI 审核推迟了一整个下午的记录捕获。该延迟模式已向你提供明显的拖延预警。'
      },
      {
        date: '2026-05-21',
        title: 'Today 工作台审核再次因为修 Lint 延后',
        source: '随手随记',
        description: '本来计划下午两点对工作台进行视觉自审，但我却花了四个多小时在研究如何完美消除 TS 编译产生的边缘 Linter 告警。这就是最经典的创造性逃避。'
      },
      {
        date: '2026-05-10',
        title: '多维关联可视化首度推迟',
        source: '随手随记',
        description: '因为对如何在画布中绘制不等高神经元的力导向算法没有十足的把握，选择在当晚找借口出去散步，拖延了多维图谱核心界面的开发。'
      }
    ]
  }
]
  ...
  return (
    ...
    >
      ...
      <header className="flex flex-col gap-1 flex-shrink-0">
        ...
        <h1 className="text-lg font-bold tracking-tight text-white">长期主题追踪</h1>
      </header>
            ...
            {TRACKED_THEMES.map((theme) => {
              ...
              return (
                ...
                >
                  ...
                  <p className={`text-xs line-clamp-2 leading-relaxed ${isSelected ? 'text-black/70 font-medium' : 'text-white/40'}`}>
                    {theme.summary}
                  </p>
                  ...
                  <div className="flex items-center justify-between w-full text-[9px] font-mono">
                    ...
                    <span className={`font-bold ${isSelected ? 'text-black/60' : 'text-white/40'}`}>
        ...
        <div className="flex-1 rounded-[6px] border border-white/5 bg-[#212121] p-5 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
          ...
          <div className="border-b border-white/5 pb-3 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-white">主题演化轨迹与关联图谱</h2>
              </div>
              <h2 className="text-base font-bold text-white mt-1.5">{activeTheme.name}</h2>
              <p className="text-xs text-white/40 mt-1 font-mono">
                首次检测到该模式浮现于: <span className="text-white/70">{activeTheme.startDate}</span> • 当前已积累 {activeTheme.recordCount} 条神经关联
              </p>
            </div>
          ...
          <div className="rounded-[6px] bg-[#000000] border border-white/5 p-4 flex flex-col gap-3">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="text-xs font-bold text-white/85">主题深度意图结晶</h3>
            </div>
          ...
          <div className="rounded-[6px] border border-white/5 bg-white/[0.01] p-4 flex flex-col gap-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-white/80">心境与行为模式神经元交织</h3>
            </div>
            <p className="text-xs text-white/50 leading-relaxed font-sans pr-1 sm:pr-4">
              {activeTheme.relationshipSummary}
            </p>
            <div className="flex flex-wrap gap-1.5 mt-0.5">
              <span className="text-[9.5px] font-mono text-white/30 self-center">高频交织心境:</span>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2 border-b border-white/5 pb-1.5">
              ...
              <span className="text-xs font-bold text-white/85">演化历史与关键变化节点</span>
            </div>
                ...
                <div key={milestone.title} className="relative">
                  ...
                  <div className="flex flex-col gap-1 pl-2">
                    <div className="flex items-center gap-2 text-[10.5px]">
                      <span className="font-mono font-bold text-white/70">{milestone.date}</span>
                      <span>•</span>
                      <span className="rounded-[6px] bg-white/5 px-1.5 py-0.2 text-[8.5px] text-white/40">
                        {milestone.source}
                      </span>
                    </div>
                    <h4 className="text-xs font-bold text-white/85">{milestone.title}</h4>
