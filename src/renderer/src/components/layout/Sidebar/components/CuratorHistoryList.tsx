...
import { CuratorHistoryContextMenu } from "@/features/curator/components/CuratorHistoryContextMenu"
 ...
 */
const sortSessionsByUpdatedTime = (items: CuratorSession[]): CuratorSession[] =>
  [...items].sort((first, second) => {
    const firstIsEmptyDraft = isEmptyCuratorDraftSession(first)
    const secondIsEmptyDraft = isEmptyCuratorDraftSession(second)

    if (firstIsEmptyDraft || secondIsEmptyDraft) {
      return firstIsEmptyDraft === secondIsEmptyDraft ? 0 : firstIsEmptyDraft ? -1 : 1
    }

    const timeDiff = getSessionTimestampValue(second.time) - getSessionTimestampValue(first.time)
...
/**
 * CuratorHistoryList - 负责左侧对话历史列表的渲染与交互
 */
export const CuratorHistoryList = ({
  sessions,
  activeSessionId,
  onSessionChange,
  onNewChat,
  ...
  isCollapsed = false,
  onCollapsedChange,
}: CuratorHistoryListProps): React.JSX.Element => {
  // 当前行内标题编辑草稿。
  const [editingTitle, setEditingTitle] = useState<EditingCuratorTitleDraft | null>(null)
  // 当前打开的右键菜单；集中管理以保证视口内只存在一个菜单。
  const [contextMenu, setContextMenu] = useState<CuratorContextMenuState | null>(null)
  // 历史搜索关键词。
  ...
  const previousSessionStatusRef = useRef<Map<string, CuratorSession["status"]>>(new Map())
  // 历史滚动容器引用，用于触底加载。
   ...
   * 进入标题行内编辑态。
   */
  const handleStartEditTitle = (session: CuratorSession): void => {
    setEditingTitle({
      id: session.id,
      title: session.title,
    })
   ...
   */
  const handleCommitEditTitle = async (session: CuratorSession): Promise<void> => {
    if (!editingTitle) {
      return
    }
   ...
   * 删除当前右键菜单指向的会话。
   */
  const handleDeleteChat = async (session: CuratorSession): Promise<void> => {
   ...
   * 切换批量选择项。
   */
  const handleSelectSession = (session: CuratorSession): void => {
    ...
    return (
      ...
      >
        ...
        <Tooltip content="新建对话" placement="right">
          <IconButton aria-label="New chat" preset="add" onClick={onNewChat} />
        </Tooltip>
      </div>
    )
  }
    ...
    >
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <Tooltip content="定位当前对话" placement="bottom">
            <IconButton aria-label="Locate active chat" onClick={handleLocateActiveSession}>
        ...
        {filteredSessions.map((session) => {
          ...
          const hasCompletionNotice =
            ...
            (completedSessionIds.has(session.id) ||
              Boolean(completionNoticeSessionIds?.has(session.id)))
            ...
            >
              <div className="flex items-start justify-between gap-2 w-full">
                ...
                {isEditing ? (
                  ...
                  <div className="min-w-0 flex-1">
                    <span
                      className={`block truncate text-xs font-bold leading-none ${isActive ? "text-white" : "group-hover:text-white"}`}
                    >
                      {session.title}
                    </span>
                    <span
                ...
                {isGenerating ? (
                  ...
                  >
                    ...
                    >
                      <span
                        className="w-[1.5px] bg-current rounded-[0.5px]"
                        style={{
                          animation: "ai-loading-bar 1s ease-in-out infinite",
                          animationDelay: "0ms",
                        }}
                      />
                      <span
      ...
      <div className="mt-auto rounded-[6px] border border-white/5 bg-white/[0.02] p-3 flex flex-col gap-1.5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-white/60 font-mono">ReAct Agent Mode</span>
        </div>
      </div>
    </div>
  )
