...
type AiChatContextState = {
  // 按会话标识存储的上下文条目。
  sessionItems: Record<string, AiChatContextItem[]>;
  // 获取指定会话上下文条目。
  getSessionItems: (sessionId: string) => AiChatContextItem[];
  // 添加单条上下文，按 key 去重。
  addItem: (item: AiChatContextItem) => void;
  // 删除指定上下文。
 ...
 * 合并上下文条目并按 key 去重。
 */
const mergeUniqueItems = (current: AiChatContextItem[], next: AiChatContextItem[]): AiChatContextItem[] => {
  const seen = new Set<string>();
  const merged: AiChatContextItem[] = [];
