 ...
 * 判断是否为分组选项
 */
const isGroup = <T,>(item: SelectOption<T> | SelectGroup<T>): item is SelectGroup<T> => {
...
export const Select = <T extends string>({
  ...
  // 根据对齐方式计算触发按钮和内容的样式。
  const buttonAlignStyles = align === "center" ? "justify-center relative" : "justify-between"

  const textAlignStyles = align === "center" ? "text-center" : "text-left"

  const chevronStyles = align === "center" ? "absolute right-2.5 top-1/2 -translate-y-1/2" : ""

  // 根据展开方向计算下拉菜单定位。
  const positionStyles = position === "up" ? "bottom-[100%] mb-1" : "top-[100%] mt-1"
  ...
  const renderOption = (option: SelectOption<T>, isGrouped: boolean) => {
    ...
    return (
      ...
      >
        <span className={`flex-1 whitespace-nowrap ${textAlignStyles}`}>{option.label}</span>
        {isSelected && <Check className="h-3 w-3 text-white ml-2 flex-shrink-0" />}
      </IconButton>
    )
  }
