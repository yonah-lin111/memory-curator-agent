...
import type { CuratorInputCommand } from "@/features/curator/components/CuratorInput/types"
...
export interface MentionCommandPanelsProps {
  ...
  // 当前匹配到的 Slash 命令列表。
  matchedCommands: CuratorInputCommand[]
  // 当前 Slash 命令面板活动焦点索引。
  activeCommandIndex: number
  // 修改 Slash 命令活动焦点索引回调。
  onActiveCommandIndexChange: (idx: number) => void
  // 确认选择 Slash 命令回调。
  onCommandSelect: (command: CuratorInputCommand) => void
  ...
  // 当前快速匹配过滤出的 AI 会话列表。
  matchedSessions: CuratorSession[]
  // 当前快速会话面板活动焦点索引。
  activeSessionIndex: number
  // 修改快速会话面板活动焦点索引回调。
  onActiveSessionIndexChange: (idx: number) => void
  // 确认选择快速会话回调。
  onSessionSelect: (session: CuratorSession) => void
  // 快速会话面板专有的键盘事件拦截器。
  ...
  return (
    <>
      <CommandPanel
        ...
        renderItem={(command) => (
          <span className="flex items-center gap-2 min-w-0">
            <span className="text-[13px] font-semibold text-white">{command.name}</span>
            <span className="text-xs text-white/30">-</span>
            <span className="truncate text-xs text-white/45">{command.description}</span>
          </span>
        )}
      />
      ...
      <CommandPanel
        ...
        renderItem={(model) => (
          <span className="flex items-center gap-2 min-w-0">
            <span className="text-[13px] font-semibold text-white">{model.modelName}</span>
            <span className="text-xs text-white/30">-</span>
            <span className="truncate text-xs text-white/45">{model.providerName}</span>
          </span>
        )}
      />
          ...
          <span className="flex items-center gap-2 min-w-0">
            ...
            <span className="truncate text-xs text-white/45">
              {session.time.includes("T")
                ? new Date(session.time).toLocaleTimeString("zh-CN", {
                    hour: "2-digit",
                    minute: "2-digit",
                    hour12: false,
                  })
        ...
        renderItem={(agent) => {
          ...
          return (
            <span className="flex items-center gap-2 min-w-0 w-full">
              <span className="text-[13px] font-semibold text-white shrink-0">{displayName}</span>
