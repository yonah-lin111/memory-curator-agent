...
// 创建联网搜索配置读取器。
const createConfigResolver = (exaApiKey = "", tavilyApiKey = "") => () => ({ exaApiKey, tavilyApiKey });
...
describe("webSearchTool", () => {
  ...
  it("配置 Tavily 后轮询调用并归一化搜索结果", async () => {
    ...
    const tool = createWebSearchTool(fetcher, new Map(), createConfigResolver("", "tavily-test-key"));

    await expect(tool.execute({ query: "first" })).resolves.toMatchObject({ data: { provider: "exa" } });
