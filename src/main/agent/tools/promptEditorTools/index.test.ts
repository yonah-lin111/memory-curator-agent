...
type ToolResult = {
  observation: string
  data: { content: string; documentHash: string; lines?: { number: number; content: string }[]; version?: number; baseVersion: number; nextVersion: number; applied: boolean }
}

/**
 * 按名称找到工具并统一执行，避免测试耦合 AgentTool 的其他字段。
 */
const executeTool = async (name: string, tools: ReturnType<typeof createPromptEditorTools>, input: unknown = {}): Promise<ToolResult> => {
...
describe('prompt editor tools', () => {
  it('inserts with exact anchors and requires a new read before another write', async () => {
    ...
    expect(inserted.data).toMatchObject({ content: 'one\nmiddle\ntwo', nextVersion: 8, applied: true })
  ...
  it('uses a unique anchor match when afterLine is off by one', async () => {
    ...
    expect(result.data).toMatchObject({ content: 'one\nmiddle\ntwo\nthree', nextVersion: 2, applied: true })
  })
  ...
  it('requires the latest document hash for line replacements', async () => {
    ...
    expect(firstRead.data.lines).toEqual([{ number: 1, content: 'first' }, { number: 2, content: 'second' }])
    ...
    expect(staleHashResult.observation).toContain('expectedDocumentHash does not match the latest read')
    ...
    expect(result.data).toMatchObject({ content: 'first\nupdated again', nextVersion: 3, applied: true })
  })
