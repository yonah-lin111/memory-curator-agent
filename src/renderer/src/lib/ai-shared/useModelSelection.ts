...
import type { CuratorModelProviderOption, CuratorModelSelection } from "@/features/curator/types"

/**
 * useCuratorModels - 专门管理 AI 模型切换选项构造、快速匹配 `/model <查询>` 过滤以及切换 AI 模型的微 Hook。
 *
  ...
  const moveActiveModel = useCallback(
    (direction: 1 | -1): void => {
      setActiveModelIndex((currentIndex) => {
        ...
        return (currentIndex + direction + matchedModels.length) % matchedModels.length
      })
    },
    [matchedModels.length],
  )
