...
export const useCuratorInput = (props: CuratorInputProps) => {
  ...
  const canSend = useMemo(() => {
    return Boolean(
      inputSendPayload.text.trim() || selectedImages.length > 0 || selectedTextFiles.length > 0,
    )
  ...
  const executeCommand = useCallback(
    (command: CuratorInputCommand): void => {
      if (command.id === "model") {
        const text = "/model "
        setInputText(text)
        resetHistoryCursor()
      ...
      if (e.key === "Backspace" && !isCommandPanelOpen && !isAgentPanelOpen) {
        ...
        if (textarea && textarea.selectionStart === textarea.selectionEnd) {
          ...
          if (skillDeletionRange) {
            ...
            // 从 store 卸载被删除的技能
            if (props.sessionId) {
              useCuratorContextStore
                .getState()
                .removeItem(props.sessionId, `skill-${skillDeletionRange.id}`)
              toast.success("已卸载技能")
            }

            requestAnimationFrame(() => {
              adjustTextareaHeight()
              textarea.setSelectionRange(skillDeletionRange.start, skillDeletionRange.start)
            })
            return
          }
