...
import type { AiModelProviderOption } from "@/features/ai-chat/types";

/**
 * useActiveAiModels - 专门用于管理当前激活 AI 模型选项加载与持久化同步的自定义 Hook。
 *
 * 实现了跨页面（如主聊天面板、Prompt 实验室）的模型选择自动同步与 localStorage 缓存读取。
 */
export const useActiveAiModels = () => {
  const [modelOptions, setModelOptions] = useState<AiModelProviderOption[]>([]);
  const [selectedModel, setSelectedModel] = useState<string>("");

  useEffect(() => {
    let isMounted = true;

    window.api?.ai?.getModelOptions?.()
      .then((options) => {
        if (!isMounted) return;
        ...
        try {
          const saved = localStorage.getItem("ai-chat-selected-model");
          if (saved) {
        ...
        const isValidModel =
          savedModel &&
          providers.some((p) =>
            p.id === savedModel!.provider &&
            p.models.some((m) => m.id === savedModel!.model)
          );
        ...
        if (isValidModel && savedModel) {
          ...
          const model =
            provider?.models.find((item) => item.id === options.defaultModel) ??
            provider?.models[0];
  ...
  const handleModelChange = (value: string) => {
    ...
    if (provider && model) {
      try {
        localStorage.setItem("ai-chat-selected-model", JSON.stringify({ provider, model }));
      } catch {
