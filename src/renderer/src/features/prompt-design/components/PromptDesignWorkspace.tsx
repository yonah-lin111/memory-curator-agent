...
// 提示词模板的可选类型。
type PromptTemplateType = "requirement" | "bug" | "refactor" | "modify-code"
...
const isInlineInputShortcut = (event: KeyboardEvent): boolean =>
  (event.ctrlKey || event.metaKey) && event.shiftKey && !event.altKey && event.code === "KeyO"
...
const PROMPT_TEMPLATES: Record<PromptTemplateType, { content: string; cursorOffset: number }> = {
  requirement: {
    content: "# 添加需求\n\n- 描述：\n- 要求：\n  - \n- 注意：\n  - ",
    cursorOffset: "# 添加需求\n\n- 描述：".length,
  },
...
const isPromptTemplateType = (id: string): id is PromptTemplateType =>
  id === "requirement" || id === "bug" || id === "refactor" || id === "modify-code"

/**
 * 根据紧凑命令查询匹配所有可独立触发的二级候选。
 */
const getFuzzySecondaryCommandOptions = (query: string): MarkdownSlashCommand[] => {
...
const getMarkdownSlashCommandOptions = (value: string): MarkdownSlashCommand[] => {
  ...
  if (/^\/template\s+/i.test(value)) {
    const typeQuery = value
      ...
      .toLowerCase()
    return PROMPT_TEMPLATE_OPTIONS.filter((command) => isFuzzyCommandMatch(typeQuery, command.id))
  }
...
const getChangeBlocks = (
  ...
  while (originalIndex < originalLines.length && candidateIndex < candidateLines.length) {
    if (originalLines[originalIndex] === candidateLines[candidateIndex]) {
      ...
      table[originalIndex + 1][candidateIndex] >= table[originalIndex][candidateIndex + 1]
    ) {
      originalIndex += 1
    } else {
      candidateIndex += 1
...
const applyChangeBlock = (content: string, block: MarkdownEditorChangeBlock): string | null => {
  ...
  const beforeContext =
    block.beforeContext ?? (block.beforeLine === undefined ? [] : [block.beforeLine])
  const afterContext =
    block.afterContext ?? (block.afterLine === undefined ? [] : [block.afterLine])
...
export const PromptDesignWorkspace = ({
  ...
  const enqueueUpdate = useCallback((id: string, designData: string): Promise<boolean> => {
    ...
    (nextContent: string): void => {
      ...
      if (pendingProgrammaticContentsRef.current.has(nextContent)) {
        ...
        if (hasSyncConflict) {
          ...
          const nextBlocks = getChangeBlocks(nextContent, nextWorkingContent).map((block) => ({
            ...block,
  ...
  const handleEditorSuggestion = useCallback(
    (workingContent: string, candidateContent: string): boolean => {
      ...
      const blocks = getChangeBlocks(contentRef.current, candidateContent).map((block) => ({
        ...block,
        id: `ai-change-${nextChangeIdRef.current++}`,
      }))
  ...
  const handleContextMenu = useCallback(
    (event: React.MouseEvent<HTMLDivElement>, view: EditorView): void => {
      ...
      if (
        pos !== null &&
        !(pos >= selection.from && pos <= selection.to && selection.from !== selection.to)
      ) {
  ...
  useEffect(() => {
    ...
    const finishInitializing = (): void => {
      const remaining = Math.max(0, MIN_SWITCH_LOADING_MS - (Date.now() - loadingStartTime))
  ...
  const syncMarkdownCommandPanel = useCallback((view: EditorView): void => {
    const commandLine = getSlashCommandLine(view)
    const commands = commandLine ? getMarkdownSlashCommandOptions(commandLine.value) : []
              ...
              keydown: (event, editorView) => {
                ...
                if (commandLine && commands.length > 0) {
                  ...
                  if (event.key === "Enter" && !event.isComposing) {
                    const command = commands[activeMarkdownCommandIndexRef.current] ?? commands[0]
                    if (command) {
                      event.preventDefault()
                      if (command.id === "new") {
                        const insert = "/new "
  ...
  const handleAcceptChange = useCallback(
    (id: string): void => {
      ...
      if (nextContent !== null) {
        ...
        const remainingBlocks = getChangeBlocks(nextContent, candidateContent).map((item) => ({
          ...item,
          id: `ai-change-${nextChangeIdRef.current++}`,
        }))
        setChangeBlocks(remainingBlocks)
        setPendingCandidateContent(remainingBlocks.length > 0 ? candidateContent : null)
        pendingCandidateContentRef.current = remainingBlocks.length > 0 ? candidateContent : null
        return
      }
      setChangeBlocks((previous) =>
        previous.map((item) => (item.id === id ? { ...item, status: "conflict" } : item)),
      )
    },
  ...
  const handleRejectChange = useCallback(
    (id: string): void => {
      ...
      const nextBlocks = getChangeBlocks(contentRef.current, nextCandidateContent).map((item) => ({
        ...item,
        id: `ai-change-${nextChangeIdRef.current++}`,
      }))
      setChangeBlocks(nextBlocks)
        ...
        <div className="min-h-0 flex-1 relative flex flex-col">
          {activeDesignId ? (
            ...
            <div className="flex h-full w-full flex-col items-center justify-center px-6 py-12 text-center animate-fade-in select-text">
              ...
              <h3 className="mb-1.5 text-sm font-bold text-white/80 font-mono">// 提示词设计</h3>
      ...
      {contextMenu && (
        <PromptDesignContextMenu
          ...
          canQuote={
            ...
            contextMenu.view.state.selection.main.from !== contextMenu.view.state.selection.main.to
          }
          ...
          onPaste={() => {
            void navigator.clipboard
              .readText()
              .then((text) =>
                contextMenu.view.dispatch(contextMenu.view.state.replaceSelection(text)),
              )
