...
type EventHandlerFixture = {
  // 运行映射引用。
  runMessageMapRef: { current: Map<string, AiRunMessageMapping> }
  // 更新消息回调。
  ...
  // 事件处理函数。
  handleEvent: ReturnType<typeof createAiChatEventHandler>
}

/**
 * 创建事件适配器测试夹具。
 */
const createFixture = (mapping: AiRunMessageMapping): EventHandlerFixture => {
  const runMessageMapRef = {
    current: new Map<string, AiRunMessageMapping>([['run-1', mapping]])
  }
  const textBufferRef = {
  ...
  const updateChatSessionStatus = vi.fn(
    (_sessionId: string, _status: AiChatSessionStatus) => undefined
  )
  const confirmOptimisticTitle = vi.fn()
  const handleEvent = createAiChatEventHandler({
    runMessageMapRef,
    textBufferRef,
    typewriterTimerRef,
    updateAiMessage,
...
describe('createAiChatEventHandler', () => {
