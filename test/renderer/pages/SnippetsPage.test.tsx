...
describe("SnippetsPage", () => {
  ...
  it("无 daily bridge 时仍可本地更新并删除片段", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Delete snippet 本地片段-已更新" }))
    await waitFor(() => {
  ...
  it("片段写接口失败时显示兜底提示并保持原状态", async () => {
    ...
    window.api.daily.createSnippet = vi.fn().mockRejectedValue(new Error("create failed"))
    window.api.daily.updateSnippet = vi.fn().mockRejectedValue(new Error("update failed"))
    window.api.daily.deleteSnippet = vi.fn().mockRejectedValue(new Error("delete failed"))
