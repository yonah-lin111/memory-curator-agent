 ...
 * 解析 Bill 更新入参。
 */
const parseUpdateInput = (input: unknown): { id: number; profile: BillUpdateInput } => {
  if (!isRecord(input)) {
 ...
 * 创建账单列表查询工具。
 */
export const createBillListTool = (billsService: Pick<BillsService, "list">): BillQueryTool => ({
  name: "bills_tool_list",
  description: "Query bills in the local Bills table. Read-only; never modifies data.",
  prompt: {
    summary:
      "Query bills in the local Bills table. Read-only; supports date ranges, category, and type filtering.",
    intentKeywords: [
      "账单",
      "消费",
      "花销",
    ...
    output:
      "Return the bill facts needed to answer the user. Format amounts to Yuan (元) for user display.",
    examples: [
      `{"limit":10}`,
  ...
  execute: async (input) => {
    ...
    const limit = Math.max(1, Math.min(parsed.limit ?? DEFAULT_BILL_LIMIT, MAX_BILL_LIMIT))
    ...
    if (items.length > 0) {
      const details = items
        .map(
          (item) =>
            `- [${item.billDate}] ${item.billType === "expense" ? "支出" : "收入"} [${item.category}]: ${formatAmount(item.amount)}${item.note ? ` (${item.note})` : ""}`,
        )
        .join("\n")
      observation += `\n\nQuery results:\n${details}`
    }
...
export const createBillSummaryTool = (
  ...
  prompt: {
    summary:
      "Query daily summary context of expense, income, and recent transactions for a specific date.",
    intentKeywords: [
      "消费摘要",
      "今天花了多少钱",
      "今日总结",
  ...
  execute: async (input) => {
    ...
    if (items.length > 0) {
      const details = items
        .map(
          (item) =>
            `- ${item.billType === "expense" ? "支出" : "收入"} [${item.category}]: ${formatAmount(item.amount)}${item.note ? ` (${item.note})` : ""}`,
        )
        .join("\n")
 ...
 * 创建账单新建工具。
 */
export const createBillAddTool = (billsService: Pick<BillsService, "create">): BillWriteTool => ({
  name: "bills_tool_add",
  ...
  prompt: {
    ...
    output:
      "Include confirmationSummary in the tool arguments; return the created bill facts needed by the user.",
    examples: [
  ...
  parameters: {
    ...
    properties: {
      confirmationSummary: {
        type: "string",
        description:
          "Concise Markdown Chinese explanation shown above the internal confirmation. Summarize the key bill facts: date, category, type, note and amount.",
      },
      amount: {
        type: "number",
...
export const createBillUpdateTool = (
  ...
  prompt: {
    ...
    intentKeywords: ["修改账单", "更新账单", "改账单", "纠正账单", "update bill", "edit bill"],
    whenToUse: [
    ...
    output:
      "Include confirmationSummary in the tool arguments with key changed fields; return the updated bill facts needed by the user.",
    examples: [
...
export const createBillDeleteTool = (
  ...
  prompt: {
    ...
    intentKeywords: ["删除账单", "删掉账单", "移除账单", "delete bill", "remove bill"],
    whenToUse: [
    ...
    examples: [`{"confirmationSummary":"将删除账单记录：#5。","id":5}`],
  },
  parameters: {
    type: "object",
    required: ["id"],
