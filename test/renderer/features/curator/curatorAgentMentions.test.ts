...
describe('curatorAgentMentions', () => {
  ...
  it('支持 agent 面板模糊匹配', () => {
    ...
    expect(getMatchedCuratorAgentMentions('peo').map((option) => option.id)).toEqual(['people', 'snippets'])
    expect(getMatchedCuratorAgentMentions('people').map((option) => option.id)).toEqual(['people'])
    expect(getMatchedCuratorAgentMentions('people[tool]').map((option) => option.id)).toEqual(['people'])
    expect(getMatchedCuratorAgentMentions('eo').map((option) => option.id)).toEqual(['people', 'snippets', 'notes'])
    expect(getMatchedCuratorAgentMentions('ty').map((option) => option.id)).toEqual(['today'])
    expect(getMatchedCuratorAgentMentions('todo').map((option) => option.id)).toEqual(['todo', 'today'])
