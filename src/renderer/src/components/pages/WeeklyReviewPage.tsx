...
const TIMELINE_MILESTONES: TimelineMilestoneItem[] = [
  {
    ...
    description:
      "Today 界面采用极纯粹的黑色与白灰色系完成，完成了对今天所有零散随记的清理，并在晚间完成了第一次本周周度回顾，整体效率进入良性循环。",
    category: "日常开发",
  },
  {
    date: "05-24",
    ...
    description:
      "反思社交平台算法对个人记忆完整性的侵蚀，确立了「Memory Curator」作为一个完全归个人所有的数字化“海马体”的底层哲学目标。",
    category: "认知觉醒",
  },
  {
    date: "05-22",
    ...
    description:
      "拒绝平庸退回 SQL 方案，在连续报错数小时后，成功用二进制碎块在本地沙盒实现神经关联衰减算法，将首屏图谱解密耗时降至 15ms。",
    category: "技术突破",
  },
  {
    date: "05-21",
    ...
    description:
      "发现自己利用修整无关紧要的 Lint 规则来逃避更核心但有些无从下手的多维关联架构。此自省作为重要样本，推动建立「计划延后模式」主题。",
    category: "情绪波动",
  },
  {
    date: "05-19",
    ...
    description:
      "在与 Yonah 进行技术研讨后，决定在主应用冷启动时只初始化 Today 精简数据，大体量的多维记忆链条采用异步分片式后台懒加载。",
    category: "日常开发",
  },
];
...
const CURATED_MEMORIES: CuratedMemoryItem[] = [
  {
    ...
    date: "2026-05-22",
    reason:
      "这是一项硬核的技术突破，验证了我们“本地优先、计算完全局限于沙盒”的核心安全原则。它决定了整个产品在工程上的最高可行性上限。",
  },
  {
    id: "m-2",
    name: "「安福路关于算法投喂的思考」",
    date: "2026-05-24",
    reason:
      "这次思考超越了纯粹的代码编写，给产品构建注入了深沉的灵魂。它回答了“我们为什么要大费周折地做一个本地优先、主观归属的脑区”这一哲学本源。",
  },
];
          ...
          <div className="rounded-[6px] border border-white/5 bg-[#212121] p-4 flex flex-col gap-3">
            ...
            <div className="bg-[#000000] border border-white/5 rounded-[6px] p-3 flex flex-col gap-3">
              <p className="text-xs text-white/70 leading-relaxed">
                本周的前半段（05-19 至 05-21）处于比较明显的
                <span className="text-white font-bold">“焦虑 / 自省”</span>
                状态，高频的随记中出现多次对自己拖延的批判，思维呈现一定的内耗与无意义重构偏向。
              </p>
              ...
              <p className="text-xs text-white/70 leading-relaxed">
                随着 05-22 深夜用 Rust
                硬核攻克多维图谱本地解密的性能难关，心境在周末显著好转，转入
                <span className="text-white font-bold">“精疲力竭 / 充实”</span>
                与周一落成工作台后的
                <span className="text-white font-bold">“平静 / 专注”</span>。
              </p>
            </div>
          </div>
              ...
              return (
                <div key={milestone.title} className="relative group">
                  ...
                  <div className="flex flex-col gap-1 pl-2">
                    ...
                    <h4 className="text-sm font-bold text-white/85 mt-0.5">
                      {milestone.title}
                    </h4>
        ...
        <div className="rounded-[6px] border border-white/5 bg-[#212121] p-4 flex flex-col gap-3 flex-shrink-0 mb-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-1">
            {CURATED_MEMORIES.map((curated) => (
              <div
                key={curated.id}
                className="rounded-[6px] bg-[#000000] border border-white/5 p-3.5 flex flex-col gap-2"
              >
                <div className="flex items-center justify-between">
