...
const buildWhere = (parsed: NoteCategoryQueryToolInput): string => {
  const parts: (string | null)[] = [
    parsed.query ? `name LIKE '%${escapeSqlLike(parsed.query.trim())}%'` : null,
  ]
...
const prepareSql = (sql: string, limit: number): string => {
  ...
  if (!/^select\b/i.test(normalized)) throw new Error("Category SQL only allows SELECT queries")
...
export const createNoteCategoryQueryTool = (
  ...
  execute: async (input) => {
    ...
    const limit = Math.max(
      1,
      Math.min(typeof parsed.limit === "number" ? parsed.limit : DEFAULT_LIMIT, MAX_LIMIT),
    )
...
const CATEGORY_ADD_CONFIRMATION: ToolConfirmationConfig = {
  ...
  confirm: "确认创建",
  cancel: "取消创建",
  renderTarget: (input) => (isRecord(input) ? ((input.name as string | undefined) ?? null) : null),
  renderSummary: (input) => {
...
export const createNoteCategoryAddTool = (
  ...
  prompt: {
    ...
    examples: ['{"confirmationSummary":"将创建分类：**工作笔记**。","name":"工作笔记"}'],
  },
  parameters: {
    type: "object",
...
export const createNoteCategoryUpdateTool = (
  ...
  prompt: {
    ...
    output: "Include confirmationSummary in the tool arguments.",
    examples: ['{"confirmationSummary":"将重命名分类为：**工作笔记**。","id":1,"name":"工作笔记"}'],
  },
  parameters: {
    type: "object",
...
export const createNoteCategoryDeleteTool = (
  ...
  prompt: {
    ...
    output: "Include confirmationSummary in the tool arguments.",
    examples: ['{"confirmationSummary":"将删除分类 #1，关联笔记的分类将被清空。","id":1}'],
  },
  parameters: {
    type: "object",
