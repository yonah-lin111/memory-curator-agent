...
export const useMarkdownFileMention = (enabled: boolean) => {
  ...
  useEffect(() => {
    ...
    window.api.promptDesign
      ?.searchFiles(activeProject.path, panelState.query)
      ?.then((files) => {
        if (isCurrent) {
              ...
              keydown: (event, view) => {
                ...
                if (isPanelOpen) {
                  if (event.key === "ArrowDown" || event.key === "ArrowUp") {
                    ...
                    setActiveIndex(
                      (index) =>
                        (index + direction + matchedFilesRef.current.length) %
                        matchedFilesRef.current.length,
                    )
                    return true
                  }
                  if (event.key === "Escape") {
                  ...
                  if (event.key === "Enter" && panelStateRef.current) {
                    ...
                    selectFileRef.current(
                      matchedFilesRef.current[activeIndexRef.current] ?? matchedFilesRef.current[0],
                    )
                    return true
                  }
                }
