...
export const registerDailyHandlers = (): void => {
  ...
  ipcMain.handle("daily:list-month-overview", (_, month: string) =>
    dailyService.listMonthOverview(month),
  )
  ...
  ipcMain.handle("daily:todo:sort", (_, input: TodoReorderInput) =>
    dailyService.reorderTodos(input),
  )
