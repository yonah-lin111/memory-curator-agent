...
// Snippet 查询字段清单。
const SNIPPET_COLUMNS = "id, entry_date, title, content, tags, created_at, updated_at"
...
const prepareSnippetSql = (sql: string, limit: number): string => {
  ...
  if (normalizedSql.includes(";") || SQL_COMMENT_PATTERN.test(normalizedSql)) {
    throw new Error("Snippet SQL only allows a single SELECT statement without comments")
  }
  ...
  if (!new RegExp(`\\bfrom\\s+${SNIPPET_TABLE_NAME}\\b`, "i").test(normalizedSql)) {
...
const queryBySql = (
  ...
  const rows = snippetsService.querySql(prepareSnippetSql(sql, limit)).slice(0, limit)
...
export const createSnippetQueryTool = (
  ...
  description: "Query snippets in the local Snippets table. Read-only; never modifies data.",
  prompt: {
    summary:
    ...
    output: "Return the snippet facts needed to answer the user. Do not repeat irrelevant fields.",
    examples: [
      `{"limit":10}`,
  ...
  execute: async (input) => {
    ...
    const limit = Math.max(1, Math.min(parsed.limit ?? DEFAULT_SNIPPET_LIMIT, MAX_SNIPPET_LIMIT))
...
const SNIPPET_ADD_CONFIRMATION: ToolConfirmationConfig = {
  ...
  completion: {
    renderMessage: (input, result) => renderSnippetMutationCompletion("add", input, result),
  },
}
...
const SNIPPET_UPDATE_CONFIRMATION: ToolConfirmationConfig = {
  ...
  completion: {
    renderMessage: (input, result) => renderSnippetMutationCompletion("update", input, result),
  },
}
...
const SNIPPET_DELETE_CONFIRMATION: ToolConfirmationConfig = {
  ...
  completion: {
    renderMessage: (input, result) => renderSnippetMutationCompletion("delete", input, result),
  },
}
 ...
 * 解析 Snippet 更新入参。
 */
const parseUpdateInput = (input: unknown): { id: number; profile: SnippetUpdateInput } => {
  if (!isRecord(input)) {
...
export const createSnippetUpdateTool = (
  ...
  prompt: {
    ...
    intentKeywords: ["修改片段", "更新片段", "纠正片段", "update snippet", "edit snippet"],
    whenToUse: [
...
export const createSnippetDeleteTool = (
  ...
  prompt: {
    ...
    intentKeywords: ["删除片段", "移除片段", "删掉片段", "delete snippet", "remove snippet"],
    whenToUse: [
    ...
    examples: ['{"confirmationSummary":"将删除代码片段：**React Hook**。","id":1}'],
  },
  parameters: {
    type: "object",
