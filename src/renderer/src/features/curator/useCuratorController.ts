 ...
 * 从启用模型列表中解析默认选择。
 */
const resolveDefaultCuratorModel = (
...
export const useCuratorController = (): UseCuratorControllerResult => {
  ...
  const handleDeleteChat = async (sessionId: string): Promise<boolean> => {
    const isDeleted = await deleteCuratorSession({
      sessionId,
      sessions: chatSessions,
      activeId: activeChatId,
  ...
  // 订阅主进程 AI 对话事件。
  useEffect(() => {
    const handleCuratorEvent = createCuratorEventHandler({
      runMessageMapRef,
      textBufferRef,
      typewriterTimerRef,
      updateAiMessage,
   ...
   */
  const startCuratorMessage = (
    input: CuratorSendInput,
    sessionId: string,
    sourceSessions: CuratorSession[] = chatSessions,
  ): void => {
    ...
    void window.api.ai
      .startChat({
        ...
        message: text,
        parts: userMessageParts,
        provider: selectedCuratorModel?.provider,
        model: selectedCuratorModel?.model,
  ...
  const handleRegenerateLatestAnswer = async (): Promise<void> => {
    await regenerateLatestCuratorAnswer({
      session: activeChatSession,
      sessions: chatSessions,
      activeId: activeChatId,
      undoLastTurn: window.api?.ai?.undoLastTurn,
