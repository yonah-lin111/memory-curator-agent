...
import { Sparkles, BookOpen, AlertTriangle, ArrowRight, HelpCircle, FileText, Link2 } from 'lucide-react'
...
const NAMED_MEMORIES: NamedMemoryNode[] = [
  {
    ...
    date: '2026-05-22',
    source: '深度日记 & 随笔',
    reason: '首度在本地 Keychain 加密和沙盒碎块文件存储的基础上，成功编译 Rust 桥接层并跑通多维关联度衰减模型，首屏渲染解密耗时仅为 15ms。这在工程上验证了我们“极致性能 + 绝对离线”的底线原则。',
    mood: '精疲力竭 / 充实'
  },
  {
    id: 'm-n2',
    ...
    date: '2026-05-24',
    source: '深度日记',
    reason: '在周末安福路午后产生的心灵火花，深刻反思了算法投喂对个人心智完整性与记忆主权的蚕食，明确提出 Memory Curator 必须作为完全归属、绝对私密之数字海马体而存活的哲学命题。',
    mood: '失落 / 警醒'
  },
  {
    id: 'm-n3',
    ...
    date: '2026-05-25',
    source: '日常开发 & 随笔',
    reason: 'Today 三栏式主工作台完成静态编码与 Lint 规则自审。酷黑配色（#000000 & #212121）、6px 圆角的极静视觉自此落入物理肉身。体验流畅自如，象征着脑区从理论变为触手可及。',
    mood: '平静 / 专注'
  }
]
...
const MEMORY_RELATIONS: MemoryRelationChain[] = [
  {
    ...
    relationDescription: '这两者共同构成我们在底层技术上的第一性原理。因为我们固执地拒绝平庸托管，在 Rust 桥接层将计算限制在本地 Keychain 沙盒中，Today 工作台在加载海量多维神经元时才能实现 120ms 的秒开，并将解密耗时锁在 15ms 内，使得渲染层无需依靠复杂的动态加载 Loading 即可流畅切换。'
  },
  {
    id: 'r-2',
    ...
    targetNodeName: '「Today 工作台酷黑自审落地」',
    relationDescription: 'Today 看板并行排布了“每日代办”、“自由笔记”以及“主观情绪日记”，这种独特的视觉呈现并不是随意的卡片拼接。它的底层逻辑来自于你在安福路对“信息主权”的反思——拒绝推荐算法喂养，还原一个供人冷静凝视、审阅自我脆弱一面的离线闭合空间。Today 即是这一哲学的物理载体。'
  }
]
...
const UNRESOLVED_ISSUES: UnresolvedIssueItem[] = [
  {
    ...
    contradictionText: '在 05-21 的随手随记中，你表达了对 AI 总结能力的激烈不信任，声称“机器的概括往往会过滤掉最主观珍贵的负面情绪，是一种粗暴的过度修饰”；但在今天（05-25）的复盘回顾中，你却对 Local Agent 自动为你归纳整理出的「计划延后拖延模式」高度认可并感到警醒。这两者之间显现出某种主观接受度的断裂。',
    actionSuggestion: '建议在下一次周整理策展中，与 Local Agent 启动一轮“人机边界自审”的闭合对话，明确理清哪些心智层次可以对本地模型开启智能结晶分析，哪些区域必须保持纯原始、无损的主观记录。'
  }
]
  ...
  return (
    ...
    >
      ...
      <header className="flex flex-col gap-1 flex-shrink-0">
        ...
        <h1 className="text-lg font-bold tracking-tight text-white">记忆片段关联墙</h1>
      </header>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 flex flex-col gap-3.5">
        ...
        <div className="rounded-[6px] border border-white/5 bg-[#212121] p-4 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-white/80">海马体的永久路标</span>
          </div>
        ...
        <div className="flex flex-col gap-2.5">
          <div className="flex items-center gap-2 px-1">
            ...
            <h2 className="text-xs font-bold text-white/80 uppercase tracking-wider">被命名的核心记忆节点</h2>
          </div>
        ...
        <div className="flex flex-col gap-2.5">
          <div className="flex items-center gap-2 px-1">
            ...
            <h2 className="text-xs font-bold text-white/75 uppercase tracking-wider">神经元关联桥梁 (为什么有关联)</h2>
          </div>
        ...
        <div className="flex flex-col gap-2.5 mb-1">
          <div className="flex items-center gap-2 px-1">
            ...
            <h2 className="text-xs font-bold text-white/75 uppercase tracking-wider">认知冲突与未完成议题警告</h2>
          </div>
          ...
          {UNRESOLVED_ISSUES.map((issue) => (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-white/80">{issue.title}</span>
              </div>
