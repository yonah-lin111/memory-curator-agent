...
export const WeeklySummaryPanel = ({ weekStartDate, isEmpty = false }: WeeklySummaryPanelProps) => {
  const { panelState, summary, streamText, streamTick, generate, hydrate, reset } =
    ...
    <div className="bg-[#212121] rounded-[6px] border border-white/5 p-4 flex flex-col h-[89vh]">
      ...
      <div className="flex-shrink-0 flex items-center justify-between border-b border-white/5 pb-2 mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold tracking-wide text-white/80">周度报告</span>
        </div>

        {panelState === "done" && (
          <div className="flex items-center gap-2">
            <Tooltip title="确认重新生成周度报告？" onConfirm={handleGenerate} placement="bottom">
              <IconButton
                disabled={isEmpty}
        ...
        {(panelState === "streaming" || panelState === "done") && (
          ...
          >
            {panelState === "done" && !isMeaningful ? (
              <p className="text-xs text-white/25 py-8 text-center">本周暂无值得总结的记录。</p>
            ) : (
              <MarkdownEditor
                ...
                placeholder="暂无周度报告"
                height="100%"
                value={panelState === "streaming" ? streamText : (summary?.content ?? "")}
                onChange={() => {}}
              />
            )}
