...
const MOCK_MEMORIES: MemoryItem[] = [
  {
    ...
    summary:
      "在开发高性能 Electron 桌面端应用时，发现 IPC 通信在进行大量大体积 JSON 传输时，会由于 V8 垃圾回收机制与 IPC 缓冲队列阻塞引发渲染进程暂时性死锁。最终解决方案是改用 SharedArrayBuffer 与本地双向 WebSocket 服务传递裸字节流，避免了 IPC 主线程的序列化负荷。",
  ...
  },
  {
    ...
    summary:
      "物理学家埃隆·马斯克崇尚的思考模型：剥离事物的表象与既定常识，将事物拆解至最基础、最无法证伪的物理学定理或底层基本真理，然后以此为基石重新向上推导。对于软件架构设计，这意味着不盲信任何所谓的行业最佳实践，而是从 I/O 吞吐、硬件限制和具体算法复杂度重新核算最精妙的极简实现。",
  ...
  },
  {
    ...
    summary:
      "在严格逆变（strictFunctionTypes）模式下，探讨了泛型方法在多态接口设计中的深层表现。参数位置具有逆变性，返回值位置具有协变性。通过自定义 `type ReadonlyDeep<T>` 类型确保深层次属性完全只读，从而防止了在子类型多态赋值时引发的静默运行时类型崩溃。",
  ...
  },
  {
    ...
    summary:
      "通过配置 PRAGMA journal_mode = WAL 以及 PRAGMA synchronous = NORMAL，极大地释放了本地 SQLite 的高并发写操作性能。即使在 50 个多线程 Agent 并发进行日志与上下文持久化时，也能确保 0ms 的读阻塞与小于 2ms 的写延迟，是本地优先（Local-First）桌面应用的绝佳数据存储基底。",
  ...
  },
  {
    ...
    summary:
      "在使用 Llama.cpp 绑定进行本地 7B 模型 4-bit 量化推理时，针对 macOS M3 Max 芯片的统一内存进行了 Metal 编程接口优化。通过调整 batch_size 到 512 并将 KV Cache 完全分配在 GPU VRAM 中，实现了每秒超过 65 个 Token 的闪电般极速生成体验。",
  ...
  },
  {
    ...
    summary:
      "高档的 UI/UX 不需要花哨的渐变或炫目的动效。通过精细的字重对比（例如 400 搭配 600）、精确到 4px/8px 倍数的间距节奏，以及绝对的主黑（#000000）与次黑（#212121）层级划分，利用微弱的白边（opacity 5%-10%）来切割空间，能让界面产生极强的现代高级感、秩序感与空间深邃感。",
...
export const App = (): React.JSX.Element => {
  ...
  const selectedMemory =
    MOCK_MEMORIES.find((m) => m.id === selectedMemoryId) || MOCK_MEMORIES[0];
      ...
      <aside className="flex w-64 flex-col justify-between rounded-[6px] border border-white/5 bg-[#212121] p-4 transition-all duration-300">
        ...
        <div className="flex flex-col gap-6">
            ...
            {MAIN_NAV_ITEMS.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 rounded-[6px] px-3 py-2.5 text-sm font-medium tracking-wide transition-all duration-200 cursor-pointer ${
                    isActive
                ...
                >
                  <IconComponent
                    className={`h-4 w-4 ${isActive ? "text-black" : "text-white/60"}`}
                  />
                  {item.label}
                </button>
              );
          ...
          <div className="flex flex-col gap-2">
            <span className="flex items-center gap-2 px-3 text-xs font-semibold tracking-widest text-white/40 uppercase">
              <FolderOpen className="h-3.5 w-3.5" />
              策展多维分类
            </span>

            {/* 可滚动分类容器 */}
            <div className="custom-scrollbar max-h-[35vh] overflow-y-auto pr-1 flex flex-col gap-1">
        ...
        {/* 底部引擎运行状态及信息 - 辅助小字体统一 12px (text-xs) */}
        <div className="mt-auto flex flex-col gap-3 pt-4 border-t border-white/5">
          ...
          <div className="flex items-center justify-between px-1 text-xs text-white/30">
      ...
      <section className="flex flex-1 gap-3 overflow-hidden">
          ...
          <div className="custom-scrollbar flex-1 overflow-y-auto p-3 flex flex-col gap-2 bg-[#000000]/20">
            {MOCK_MEMORIES.map((item) => {
              ...
              return (
                ...
                >
                  {/* 卡片头部：来源与时间 - 统一辅助小字体 12px (text-xs) */}
                  ...
                  <div className="flex flex-wrap gap-1 mt-0.5">
                    ...
                    {item.tags.length > 3 && (
                      <span className="text-xs text-white/30 flex items-center px-1">
                        +{item.tags.length - 3}
                      </span>
                    )}
                  </div>
        ...
        <div className="flex flex-1 flex-col rounded-[6px] border border-white/5 bg-[#212121] overflow-hidden">
          ...
          <div className="custom-scrollbar flex-1 overflow-y-auto p-6 flex flex-col gap-6">
            ...
            {/* 提示与系统行动指示 (Decorative System block) - 辅助性描述字号 12px (text-xs) */}
            <div className="rounded-[6px] border border-white/5 bg-white/2 p-4.5 flex flex-col gap-2.5">
              ...
              <p className="text-xs leading-relaxed text-white/40 font-light">
                该本地记忆实体在 “{selectedMemory.tags[0]}”
                节点链中权重较高。系统建议将其作为上下文快照在下一次 LLM
                提示词合成中自动关联，以增强 Agent
                在相关软件架构及系统调优上的精准度和召回深度。
              </p>
            </div>
          </div>
        </div>
