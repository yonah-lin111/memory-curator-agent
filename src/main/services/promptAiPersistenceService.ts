...
export class PromptAiPersistenceService {
  ...
  public ensureSession(input: EnsurePromptAiSessionInput): void {
    if (input.designItemId === "default-design-item-id") {
      const checkProject = this.db
        .prepare("SELECT id FROM prompt_design_projects WHERE external_id = 'default-project-id'")
        .get()
      if (!checkProject) {
        this.db
          .prepare(`
      ...
      const checkDesign = this.db
        .prepare("SELECT id FROM prompt_design_items WHERE external_id = 'default-design-item-id'")
        .get()
      if (!checkDesign) {
        this.db
          .prepare(`
    ...
    if (!existing) {
      ...
      const updateStmt = this.db.prepare(`
        ...
        WHERE external_id = ?
      `)
      updateStmt.run(input.title, input.status, input.timestamp, input.timestamp, input.id)
    }
  }
  ...
  public getSession(sessionId: string): PromptAiChatSessionItem | null {
    ...
    `)
    const sessionRow = sessionStmt.get(sessionId) as PromptAiChatSessionRow | undefined
    ...
    const messages = messageRows.map((row) => {
      const parts = row.parts_json ? (JSON.parse(row.parts_json) as AiChatMessagePart[]) : []
      const toolSteps = row.tool_steps_json ? (JSON.parse(row.tool_steps_json) as AiToolStep[]) : []
  ...
  public deleteSession(sessionId: string): void {
    const stmt = this.db.prepare("DELETE FROM prompt_ai_chat_sessions WHERE external_id = ?")
    stmt.run(sessionId)
  }
  ...
  public undoLastTurn(sessionId: string): PromptAiChatSessionItem | null {
    const undoTx = this.db.transaction(() => {
      ...
      `)
      const messages = messagesStmt.all(sessionId) as {
        row_order: number
        external_id: string
        role: string
        created_at: string
      ...
      const turnStartIndex = [...messages]
        .reverse()
        .findIndex((message) => message.role === "user" || message.role === "system_command")
      ...
      if (removedAssistantMessageIds.length > 0) {
        ...
        this.db
          .prepare(
            `DELETE FROM prompt_ai_agent_runs WHERE session_id = ? AND assistant_message_id IN (${placeholders})`,
          )
          .run(sessionId, ...removedAssistantMessageIds)
      }

      if (removedMessageIds.length > 0) {
        const placeholders = removedMessageIds.map(() => "?").join(", ")
        this.db
          .prepare(`DELETE FROM prompt_ai_chat_messages WHERE external_id IN (${placeholders})`)
  ...
  public deleteTurnByMessageId(
    ...
    const deleteTx = this.db.transaction(() => {
      ...
      this.updateSessionTimestamp(
        sessionId,
        remainingMessages.at(-1)?.created_at ?? new Date().toISOString(),
      )
    })
  ...
  public upsertToolSteps(
    ...
    const stmt = this.db.prepare(`
      ...
      WHERE external_id = ?
    `)
    stmt.run(JSON.stringify(toolSteps), JSON.stringify(parts), timestamp, messageId)
  }
  ...
  public updateAgentRunStatus(runId: string, status: AiAgentRunStatus, error?: string): void {
    const timestamp = new Date().toISOString()
    const stmt = this.db.prepare(`
