...
describe("curatorContextStore", () => {
  beforeEach(() => useCuratorContextStore.getState().resetAll())

  it("按会话隔离并按 key 去重", () => {
    const store = useCuratorContextStore.getState()
    store.addItem(item)
    store.addItem(item)
    store.addItem({ ...item, key: "message:m2", sourceId: "m2", sessionId: "s2" })

    expect(useCuratorContextStore.getState().getSessionItems("s1")).toHaveLength(1)
    expect(useCuratorContextStore.getState().getSessionItems("s2")).toHaveLength(1)
  })
  ...
  it("同步消息上下文时保留非消息上下文", () => {
    ...
    const items = useCuratorContextStore.getState().getSessionItems("s1")
  ...
  it("支持删除和清空会话", () => {
    ...
    expect(useCuratorContextStore.getState().getSessionItems("s1")).toHaveLength(0)
  ...
  it("限制全局会话上下文缓存数量并保留最近会话", () => {
    ...
    expect(useCuratorContextStore.getState().getSessionItems("s1")).toHaveLength(0)
    expect(useCuratorContextStore.getState().getSessionItems("s2")).toHaveLength(1)
    expect(Object.keys(useCuratorContextStore.getState().sessionItems)).toHaveLength(20)
  })
})
